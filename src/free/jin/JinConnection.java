...
public interface JinConnection{
   ...
   * if the server this JinConnection is for doesn't support the wild variant
   * to which the type of the Move corresponds. It doesn't throw a checked exception
   * as this situation shouldn't happen because the start of a game is supposed to
   * be initiated by the JinConnection, which shouldn't create games with wild
   * variants it doesn't support.
   */

  void makeMove(Game game, Move move) throws IllegalArgumentException;
