...
/**
 * A preferences panel allowing the user to specify the set of custom consoles
 * he will be using.
 * 
...
public abstract class CustomConsolesPrefsPanel extends PreferencesPanel{
  ...
  /**
   * The text field where the user can specify a regular expression such that
   * any matching messages are displayed in the console.
   */
  ...
  /**
   * This flag is set to when the values of the console properties UI are being
   * changed, but no console properties should be updated (for example, because
   * the selected console changed). This is done so that the corresponding
   * listener knows not to fire a <code>ChangeEvent</code>.
   */
  ...
  /**
   * This flag is set when changes in the console selection are to be ignored.
   * This happens, for example, when a console is being deleted and the
  ...
  /**
   * We set this to <code>true</code> after invoking {@link #createLayout()}
   * to know not to invoke it again.
   */
  ...
  public CustomConsolesPrefsPanel(ConsoleManager consoleManager){
    ...
    for (int i = prefs.getInt("consoles.count") - 1; i > 0; i--){
      ...
      if (!"custom".equals(prefs.getString(prefix + "type")))
        break;
    ...
    removeConsoleButton.addActionListener(new ActionListener(){
      public void actionPerformed(ActionEvent e){
        ...
        try{
          ...
          if (selectedIndex < consoles.size())
            consolesList.setSelectedIndex(selectedIndex);
  ...
  /**
   * Updates the console title UI from the specified selected spec (which may
  ...
  /**
   * Upates the console channels UI from the specified selected spec (which may
   * be <code>null</code>, if there is no spec selected).
   */
  ...
  /**
   * Updates the match regex UI from the specified selected spec (which may be
   * <code>null</code>, if there is no spec selected).
   */
  ...
  /**
   * Creates a string to be displayed in the channels field for the specified
   * channels.
   */
  ...
  /**
   * Parses the string displayed in the channels field and returns the
   * corresponding list of channels. Throws a {@link BadChangesException} if the
   * value is badly formatted.
   */
  
  protected abstract List parseChannelsListDisplayString(String channels) throws BadChangesException;
  ...
  /**
   * Returns whether we are temporarily ignoring changes in the console
   * properties UI.
   */
  ...
  /**
   * Sets whether we are to temporarily ignore changes in the console properties
   * UI. This is used when the changes don't indicate a real change in the
   * settings of a console, such as when the selected console changes, and we
   * update the UI. 
   */
  ...
  /**
   * Sets whether we are to temporarily ignore changes in the console selection.
   * This is used when such changes are done programmatically, in a controlled
   * manner, and don't require the default response (for example, when a console
   * is being removed).
   */
  ...
  /**
   * Creates a <code>ConsoleSpec</code> for a new console. This method may be
   * overridden to return a server-specific type. This is the only method used
   * to create new <code>ConsoleSpec</code>s, so the subclass is guaranteed to
   * ever only having to deal with the type it creates.
   */
  ...
  /**
   * Loads a <code>ConsoleSpec</code> with the specified prefix from
   * preferences.
   */
  ...
  /**
   * Stores the specified <code>ConsoleSpec</code> into preferences, using the
   * specified prefix.
   */
  ...
  /**
   * Returns the string to be displayed in the channels add/remove popup for the
   * specified channel.
   */
  ...
  protected static class ConsoleSpec{
    ...
    private static final String DEFAULT_TITLE = I18n.get(CustomConsolesPrefsPanel.class).getString("defaultConsoleTitle");
    ...
    /**
     * The regular expression that messages are matched against to determine
     * whether they are displayed in the console; <code>null</code> if none.
     */
  ...
  /**
   * The popup we use to display the list of channels for the user to
   * add/remove.
   */
  ...
  private class ChannelsPopup extends BasicComboPopup{
    ...
    /**
     * Creates a new <code>ChannelsPopup</code> with the specified list of
     * channels.
     */
    ...
    /**
     * Adds an action listener to receive notifications when an item has been
    ...
    /**
     * Removes an action listener from receiving notifications when an item has
     * been selected in the popup. 
     */
