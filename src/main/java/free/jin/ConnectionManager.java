...
public class ConnectionManager {
  ...
  /**
   * Displays UI which allows the user to login to one of the supported servers. This method is
   * invoked (usually by the class responsible for the start-up of the application) when all the
   * start up initialization (preferences, ui factory etc.) has finished and it's ok to start the
  ...
  private Server findLoginServer() {
    ...
    if (lastUser != null) return lastUser.getServer();

    // Sole supported server
    if (jin.getServers().size() == 1) return Iterables.getOnlyElement(jin.getServers());

    // Server choice panel
    Server askedServer = new ServerChoicePanel().askServer();
    if (askedServer != null) return askedServer;
  ...
  private User findLoginUser(Server server) {
    ...
    if (isGuest) return server.getGuest();
    ...
    // Last logged in user
    User lastUser = loadLastUser();
    if ((lastUser != null) && (lastUser.getServer() == server)) return lastUser;

    // Only one known user on the server
    List serverUsers = jin.getUsers(server);
    if (serverUsers.size() == 1) return (User) serverUsers.get(0);
  ...
  private ConnectionDetails findConnDetails(Server server, User user) {
    ...
    int[] ports;
    if (portsString != null) ports = TextUtilities.parseIntList(portsString, ",");
    else ports = connDetails.getPorts();
    ...
    if (user.isGuest())
      ...
      connDetails =
          ConnectionDetails.create(
              server, user, user.getUsername(), password, savePassword, hostname, ports);
  ...
  public void displayNewConnUI() {
    ...
    // Sole supported server
    if (servers.size() == 1) server = Iterables.getOnlyElement(servers);
  ...
  /**
   * Closes the current session, if any, and optionally reconnects using the connection details of
   * the current session.
   */
  ...
  private User loadLastUser() {
    ...
    if (username == null) return server.getGuest();
  ...
  private class ReconnectDialogPanel extends DialogPanel implements ConnectionListener {
    ...
    /**
     * Whether the session is closing. If a SESSION_CLOSING event is fired before the
     * CONNECTION_LOST event, it means the connection was lost due to a user closing the session, so
     * we shouldn't show ourselves.
     */
    private boolean sessionClosing = false;

    /**
     * Creates a new <code>ReconnectDialogPanel</code> for the specified session. The dialog will
     * listen to session events and display itself when appropriate.
     */
      ...
      addSessionListener(
          new SessionListener() {
            ...
            public void sessionClosing(SessionEvent evt) {
              ...
              if (isShowing()) close(null);
              else sessionClosing = true;
            }
