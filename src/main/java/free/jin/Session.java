...
public class Session {
  ...
  /**
   * Creates a new <code>Session</code> with the specified <code>ConnectionDetails</code>. The
   * <code>Session</code> is not initially connected - you must invoke the {@link #login()} method
   * to connect and log in.
   *
   * @throws PluginStartException creating, initializing or starting one of the plugins fails.
   */
  public Session(ConnectionDetails connDetails) throws PluginStartException {
    if (connDetails == null) throw new IllegalArgumentException("connDetails may not be null");
  ...
  /**
   * Creates the standalone actions for this session and returns an array of them. Yes, I shouldn't
   * reuse PluginStartException here, but so what :-p.
   */
  private JinAction[] createActions() throws PluginStartException {
    ...
    for (int i = 0; i < actionsInfo.length; i++) {
      ...
      if (action.setContext(actionContext)) actions.addElement(action);
      else System.err.println("Warning: unsupported action not created - " + action.getId());
    }
  ...
  private Plugin[] createPlugins() throws PluginStartException {
    ...
    pluginContext = new PluginContext(conn, getUser(), plugins, pluginPrefs, actions);
  ...
  /**
   * Returns whether we are currently actually connected to the server. Note that we may be
   * disconnected but the session isn't closed yet.
   */
  ...
  /**
   * Closes this session, disconnecting from the server (if necessary) and stopping all the plugins.
   */
  public synchronized void close() {
    for (int i = 0; i < plugins.length; i++) plugins[i].saveState();

    for (int i = 0; i < plugins.length; i++) plugins[i].stop();

    try {
      if (conn.isConnected()) conn.exit();
  ...
  /**
   * Initiates connection and login and then listens for results. If login fails, informs
   * ConnectionManager.
   */
  private class ConnectionInitializer implements ConnectionListener {
    ...
    /**
     * Creates a new ConnectionInitializer with the specified hostname to connect to and the
     * specified ports to connect on.
     */
    ...
    /**
     * Tries to connect on the next port, or, if all ports have been tried, notifies the
     * ConnectionManager.
     */
    @Override
