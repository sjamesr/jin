...
abstract class ScriptDialog extends JDialog {
  ...
  public ScriptDialog(Component parent, String title, Scripter scripter, Script templateScript) {
    ...
    if (scripter == null) throw new IllegalArgumentException("The specified scripter is null");
  ...
  /**
   * This method is called when the user clicks the OK button. It is supposed to collect the script
   * type specific data from the UI and return a script. If the user specified information is
   * invalid, it should handle the problem in some manner (displaying an error message to the user,
   * for example) and return null.
   */
  ...
  protected final void createUI() {
    ...
    String defaultEventType =
        (templateScript == null ? SELECT_EVENT_TYPE : templateScript.getEventType());
    ...
    for (int i = 0; i < eventTypes.length; i++)
      eventTypeChoiceEntries[i] =
          new NamedWrapper(
              eventTypes[i], scripterI18n.getString("eventTypeNames." + eventTypes[i]));
    ...
    eventTypeChoice.addItem(
        new NamedWrapper(SELECT_EVENT_TYPE, i18n.getString("selectEventTypeString")));
    ...
    eventTypeChoice.addActionListener(
        new ActionListener() {
          @Override
    ...
    eventTypeHelp.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            I18n i18n = I18n.get(ScriptDialog.class);
    ...
    eventSubtypesHelp.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            I18n i18n = I18n.get(ScriptDialog.class);
    ...
    if (!SELECT_EVENT_TYPE.equals(eventType)) {
      ...
      if (subtypes != null) {
        ...
        for (int i = 0; i < subtypes.length; i++) {
          ...
          JCheckBox checkbox =
              new JCheckBox(
                  scripterI18n.getString("eventSubtypeNames." + eventType + "." + subtype));
  ...
  /**
   * Displays the dialog until the user finishes creating the script and then returns the resulting
   * <code>Script</code>.
   */
