...
class BeanShellScriptDialog extends ScriptDialog {
  ...
  /**
   * Creates a new <code>BeanShellScriptDialog</code> with the specified parent and <code>Script
   * </code> whose properties are used as default values.
   */
  public BeanShellScriptDialog(
      Component parent, Scripter scripter, BeanShellScript templateScript) {
  ...
  protected Script createScriptOnOk(
    ...
    try {
      BeanShellScript script =
          new BeanShellScript(scripter, scriptName, eventType, selectedSubtypes, code);

      if (templateScript != null) script.setEnabled(templateScript.isEnabled());
      return script;
    } catch (EvalError e) {
      I18n.get(BeanShellScriptDialog.class)
