...
/**
 * A <code>JinEventListener</code> implementation which runs the specified piece of BeanShell code.
 */
public class BeanShellScript extends Script {
  ...
  /**
   * This is a method which is never called whose purpose is to ensure that all the classes and
   * methods used in this script via BeanShell actually exist. If they are removed by mistake, a
   * compile-time error should occur.
   */
  public static void ensureCorrectness(Scripter scripter) throws java.io.IOException {
    if ("".length() == 0) throw new Error("ensureCorrectness must never be invoked");
  ...
  public void run(JinEvent event, String eventSubtype, Object[][] vars) {
    try {
      ...
      for (int i = 0; i < vars.length; i++) {
        ...
        if (varValue instanceof Integer) bsh.set(varName, ((Integer) varValue).intValue());
        else if (varValue instanceof Long) bsh.set(varName, ((Long) varValue).longValue());
        else if (varValue instanceof Double) bsh.set(varName, ((Double) varValue).doubleValue());
        else if (varValue instanceof Float) bsh.set(varName, ((Float) varValue).floatValue());
        else if (varValue instanceof Boolean) bsh.set(varName, ((Boolean) varValue).booleanValue());
        else bsh.set(varName, varValue);
      }
  ...
  public Script createCopy() {
    try {
      BeanShellScript script =
          new BeanShellScript(scripter, getName(), getEventType(), getEventSubtypes(), getCode());
