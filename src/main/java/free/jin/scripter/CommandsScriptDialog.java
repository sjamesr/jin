...
class CommandsScriptDialog extends ScriptDialog {
  ...
  protected Container createScriptTypeSpecificUI() {
    ...
    String[] defaultCommands =
        (templateScript == null ? new String[0] : templateScript.getCommands());
    ...
    for (int i = 0; i < defaultCommands.length; i++) commandsArea.append(defaultCommands[i] + "\n");
    ...
    commandsScrollPane.setHorizontalScrollBarPolicy(
    ...
    conditionHelp.addActionListener(
        new ActionListener() {
          @Override
    ...
    commandsHelp.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            I18n i18n = I18n.get(CommandsScriptDialog.class);
  ...
  protected Script createScriptOnOk(
    ...
    for (int i = 0; tokenizer.hasMoreTokens(); i++) commands[i] = tokenizer.nextToken();

    if ((condition == null) || (condition.length() == 0)) condition = "true";
    ...
    try {
      ...
      if (!(val instanceof Boolean)) throw new EvalError("Not a boolean expression", null, null);
    } catch (EvalError e) {
      I18n i18n = I18n.get(CommandsScriptDialog.class);
      Object result =
          i18n.confirm(
              OptionPanel.OK,
              "malformedConditionDialog",
              this,
              new Object[] {format(e.getMessage())});

      if (result != OptionPanel.OK) return null;
    }

    try {
      CommandScript script =
          new CommandScript(scripter, scriptName, eventType, selectedSubtypes, condition, commands);

      if (templateScript != null) script.setEnabled(templateScript.isEnabled());
      return script;
    } catch (EvalError e) {
      I18n.get(CommandsScriptDialog.class)
          .error("malformedScriptDialog", this, new Object[] {e.getErrorText()});
      return null;
    }
  }
