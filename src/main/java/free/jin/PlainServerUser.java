...
/**
 * A simple implementation of <code>ServerUser</code>, which keeps the username as a property and
 * may be set-up as case-sensitive or case-insensitive.
 */
public class PlainServerUser implements ServerUser {
  ...
  /**
   * Creates a new <code>PlainServerUser</code> with the specified case-sensitive or
   * case-insensitive user name/handle/nickname.
   */
  ...
  /**
   * Returns whether the specified object has the same class as this one, and its name matches the
   * name of this user.
   */
  @Override
  public boolean equals(Object o) {
    ...
    if (isCaseSensitive != user.isCaseSensitive) return false;

    if (isCaseSensitive) return name.equals(user.name);
