...
/**
 * The <code>Server</code> implementation representing the chessclub.com server. See
 * http://www.chessclub.com for more information.
 */
  ...
  protected UsernamePolicy createUsernamePolicy() {
    return new UsernamePolicy() {
      ...
      public String invalidityReason(String username) {
        ...
        if (lastChar == '-') return getI18n().getString("usernameLastCharHyphenErrorMessage");
        ...
        for (int i = 0; i < usernameLength; i++) {
          ...
          if (c == '-') {
            ...
            else hasHyphenAppeared = true;
          }

          int val = c;
          if (!(isLetter(val) || isDigit(val) || (c == '-')))
            return getI18n()
                .getFormattedString("usernameIllegalCharacterErrorMessage", new Object[] {"" + c});
        }
