...
public class PluginContainersMenu extends JMenu
  ...
  /**
   * Maps <code>PluginUIContainers</code> to <code>JCheckBoxMenuItems</code> which allow
   * showing/hiding them.
   */
  private final Hashtable containersToVisCheckBoxes = new Hashtable();

  /**
   * Maps <code>JCheckBoxMenuItems</code> that allow showing/hiding plugin ui containers to the said
   * containers.
   */
  private final Hashtable visCheckBoxesToContainers = new Hashtable();

  /**
   * Maps <code>PluginUIContainers</code> to <code>JRadioButtons</code> that control which container
   * is currently active.
   */
  private final Hashtable containersToActiveRadioButtons = new Hashtable();

  /**
   * Maps <code>JRadioButtons</code> that control which container is currently active to the said
   * containers.
   */
  private final Hashtable activeRadioButtonsToContainers = new Hashtable();

  /**
   * The list of all menu items (including the separator) in this menu. We manage our own list
   * because on every add/remove of an item we need to rebuild the real menu. That we have to do
   * because JMenu.add(Component, int index) is completely broken under Swing 1.1.1.
   */
  private final Vector items = new Vector();

  /**
   * The position of the separator between checkboxes for showing/hiding containers and radio
   * buttons for activating containers. -1 if none.
   */
  ...
  /**
   * Creates a new <code>PluginContainersMenu</code> with the specified list of existing plugin ui
   * containers.
   */
  ...
  private void syncMenus() {
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      if (item instanceof JMenuItem) add((JMenuItem) item);
      else if (item instanceof JSeparator) addSeparator();
    }
  }
  ...
  public void pluginContainerAdded(PluginUIContainer pc) {
    ...
    if (pc.isVisible()) addActiveRadioButton(item);
  ...
  public void pluginUIShown(PluginUIEvent evt) {
    ...
    if (checkBox != null) checkBox.setState(true);

    JRadioButtonMenuItem radioButton =
        (JRadioButtonMenuItem) containersToActiveRadioButtons.get(evt.getSource());
  ...
  public void pluginUIHidden(PluginUIEvent evt) {
    ...
    if (item != null) item.setState(false);
  ...
  public void pluginUITitleChanged(PluginUIEvent evt) {
    ...
    if (checkBox != null) syncShowCheckBoxText(checkBox, pc);

    JRadioButtonMenuItem radioButton =
        (JRadioButtonMenuItem) containersToActiveRadioButtons.get(pc);
    radioButton.setText(pc.getTitle());
  }

  /**
   * This method is invoked when the user performs a ui closing operation on one of the current
