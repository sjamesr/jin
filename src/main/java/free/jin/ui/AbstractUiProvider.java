...
public abstract class AbstractUiProvider implements UIProvider, SessionListener {
  ...
  /**
   * Maps Pair<String, String> (<pluginId, containerId>) to the <code>PluginContainers</code> for
   * that plugin and container id.
   */
      ...
      if (container.getMode() == HIDEABLE_CONTAINER_MODE)
        container.setVisible(
            container
                .getPlugin()
                .getPrefs()
                .getBool(container.getPrefsPrefix() + "visible", container.isVisibleFirstTime()));
    }
  }
    ...
    while (pluginContainers.hasMoreElements()) {
      ...
      if (container.getMode() == HIDEABLE_CONTAINER_MODE)
        container
            .getPlugin()
            .getPrefs()
            .setBool(container.getPrefsPrefix() + "visible", container.isVisible());
    }
  ...
  private void loadSelectedFrame(Session session) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (c.getPlugin().getId().equals(pluginId) && c.getId().equals(containerId)) {
        if (c.isVisible()) c.setActive(true);
        break;
      }
    }
  }
  ...
  /**
   * Adds the specified plugin ui container to the table of <code>pluginContainers</code>. This
   * method should be called by the implementation's <code>createPluginUIContainer</code> method.
   */
  protected final void addPluginContainer(
      Plugin plugin, String id, AbstractPluginUIContainer container) {
    if (id != null) {
    ...
    for (int i = 0; i < listeners.length; i += 2) {
      if (listeners[i] == PluginUIContainerCreationListener.class) {
        PluginUIContainerCreationListener listener =
            (PluginUIContainerCreationListener) listeners[i + 1];
  ...
  /**
   * Returns the plugin ui container with the specified container id, for the plugin with the
   * specified id.
   */
  ...
  /**
   * Adds a <code>PluginUIContainerCreationListener</code> to be notified when a new plugin UI
  ...
  /**
   * Removes a <code>PluginUIContainerCreationListener</code> from being notified when a new plugin
  ...
  /**
   * Disposes of all the plugin containers and removes them from the <code>pluginContainers</code>
   * hashtable.
   */
  protected void removePluginContainers() {
    Enumeration e = pluginContainers.elements();
    while (e.hasMoreElements()) ((PluginUIContainer) e.nextElement()).dispose();
  ...
  public static void restoreWindowGeometry(
    ...
    if (relativeFrameBounds == null) {
      ...
      relativeFrameBounds =
            new RectDouble(oldFrameBounds).scale(1d / screenSize.width, 1d / screenSize.height);
      else relativeFrameBounds = defaultFrameBounds;
    }
    Rectangle realFrameBounds =
        relativeFrameBounds.scale(screenSize.width, screenSize.height).toRect();
  ...
  /**
   * Saves the geometry of the specified window into the preferences with preference names prefixed
  ...
  public static void saveWindowGeometry(Preferences prefs, Window window, String prefNamePrefix) {
    ...
    RectDouble frameBounds =
        new RectDouble(frameLocation.x, frameLocation.y, frameSize.width, frameSize.height);
  ...
  /**
   * Returns whether the specified window bounds are reasonably placed on a screen of the specified
   * dimensions. This is used to avoid situations where a window is displayed outside of the screen
   * where the user can't change its size and/or move it (can happen for example if the resolution
   * is changed between runs).
   */
  public static boolean windowBoundsOk(Dimension screenSize, Rectangle windowBounds) {
    if (windowBounds.x + windowBounds.width < 50) return false;
    if (windowBounds.y < -10) return false;
    if (windowBounds.width < 30) return false;
    if (windowBounds.height < 40) return false;
    if (windowBounds.x > screenSize.width - 10) return false;
    if (windowBounds.y > screenSize.height - 20) return false;
