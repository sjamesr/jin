...
public class PrefsMenu extends JMenu implements SessionListener {
  ...
  /**
   * The index of the separator between global preferences and plugin preferences. <code>-1</code>
   * when none.
   */
        ...
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            JDialog dialog =
                new PrefsDialog(
                    parentFrame,
                    I18n.get(PrefsMenu.class).getString("userInterfacePrefsDialog.title"),
                    new UiPrefsPanel());
  ...
  public void setConnected(boolean isConnected, Session session) {
    ...
    if (isConnected) addPluginPreferenceMenuItems();
  ...
  private void addPluginPreferenceMenuItems() {
    ...
    for (int i = 0; i < plugins.length; i++) {
      ...
      JMenuItem menuItem =
          I18n.get(PrefsMenu.class)
              .createMenuItem("pluginPrefsMenuItem", new Object[] {plugin.getName()});
            ...
            public void actionPerformed(ActionEvent evt) {
              ...
              String pluginPrefsDialogTitle =
                  i18n.getFormattedString(
                      "pluginPrefsDialog.title", new Object[] {plugin.getName()});

              Frame parentFrame = AWTUtilities.frameForComponent(PrefsMenu.this);
              JDialog dialog =
                  new PrefsDialog(parentFrame, pluginPrefsDialogTitle, plugin.getPreferencesUI());
              AWTUtilities.centerWindow(dialog, parentFrame);
              dialog.setVisible(true);
            }
          });
  ...
  /**
   * Removes the menu items associated with displaying the preference panels of the various plugins.
   */
  ...
  private class PrefsDialog extends JDialog implements ChangeListener, ActionListener {
    ...
    /**
     * Creates a new <code>PrefsDialog</code> with the specified parent frame, title and preferences
     * panel.
     */
    ...
    private void createUI() {
      ...
      if (PlatformUtils.isMacOSX()) bottomPanel.setBorder(new EmptyBorder(10, 10, 10, 10));
    ...
    /**
     * ChangeListener implementation. Registered with the preferences panel, enables the apply
     * button when invoked.
     */
    @Override
    ...
    public void actionPerformed(ActionEvent evt) {
      try {
        if (applyButton.isEnabled()) {
          ...
          if (requiresRestart) {
            Object result =
                I18n.get(PrefsDialog.class)
                    .question(
                        OptionPanel.YES, "restartDialog", this, new Object[] {Jin.getAppName()});

            if (result == OptionPanel.YES) Jin.getInstance().quit(false);
          }
        }
        applyButton.setEnabled(false);

        if (evt.getSource() == okButton) dispose();
