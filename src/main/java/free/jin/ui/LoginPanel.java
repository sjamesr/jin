...
public class LoginPanel extends DialogPanel {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            if (usernameBox.getSelectedItem() instanceof User)
              ...
              if (invalidityReason != null) {
                OptionPanel.error(
                    i18n.getString("invalidUsernameDialog.title"),
                    invalidityReason,
                    LoginPanel.this);
                return;
              }
              ...
              if (user == null) {
                if (policy.isSame(username, policy.getGuestUsername())) user = server.getGuest();
            ...
            boolean savePassword =
                (savePasswordCheckBox != null) && savePasswordCheckBox.isSelected();

            ConnectionDetails result =
                user.isGuest()
                    ? ConnectionDetails.createGuest(
                        server, user.getUsername(), (String) hostnameBox.getSelectedItem(), ports)
                    : ConnectionDetails.create(
                        server,
                        user,
                        user.getUsername(),
                        password,
                        savePassword,
                        (String) hostnameBox.getSelectedItem(),
                        ports);
    ...
    serverBox.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            else setData(server, null, false, true);
          }
        });

    usernameBox.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            if (selectedIndex == -1) setData(server, null, false, false);
  ...
  private void setData(
    ...
    if (updateServer) serverBox.setSelectedIndex(servers.indexOf(server));
    ...
    if (updateUsername) {
      ...
      User user;
      if (connDetails == null) user = null;
      else if (connDetails.isGuest()) user = server.getGuest();
      else user = Jin.getInstance().getUser(server, connDetails.getUsername());

      if (user == null) usernameBox.setSelectedItem("");
      else usernameBox.setSelectedIndex(Utilities.indexOf(users, user) + 1);
    }
    ...
    JComponent[] guestDependentComponents =
        new JComponent[] {passwordLabel, passwordField, savePasswordCheckBox, retrievePasswordLink};
    ...
    hostnameBox.setSelectedItem(
        connDetails == null ? server.getDefaultHost() : connDetails.getHost());
    ...
    if ("".equals(usernameBox.getSelectedItem())) focusedComponent = usernameBox;
    else if ((passwordField.getPassword().length == 0) && passwordField.isEnabled())
      focusedComponent = passwordField;
    else focusedComponent = connectButton;

    if (isShowing()) focusedComponent.requestFocusInWindow();
  ...
  private User[] getServerUsers(Server server) {
    ...
    for (int i = 0; i < usersList.getSize(); i++) {
      ...
      if (user.getServer() == server) usersVec.addElement(user);
    }
  ...
  private ComboBoxModel createAccountBoxModel() {
    ...
    for (int i = 0; i < users.length; i++) model.addElement(users[i].getUsername());
  ...
  private void createUI() {
    ...
    MoreLessOptionsButton moreLessButton =
        new MoreLessOptionsButton(
            false, new Component[] {hostnameLabel, hostnameBox, portsLabel, portsField});
        ...
        layout
            ...
            .add(
                layout
                    ...
                    .add(hostnameBox))
            .addPreferredGap(LayoutStyle.RELATED)
            .add(layout.createParallelGroup(GroupLayout.BASELINE).add(portsLabel).add(portsField))
  ...
  /**
   * Parses the specified string as a list of space delimited ports. Returns <code>null</code> if
  ...
  private static int[] parsePorts(String s) {
    ...
    try {
      for (int i = 0; i < ports.length; i++) {
        ...
        if ((port < 0) || (port > Short.MAX_VALUE)) return null;
        ports[i] = port;
      }
