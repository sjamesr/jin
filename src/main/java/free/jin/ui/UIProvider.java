 ...
 * rest of the application. An implementation might, for example, choose to use a single main frame
 * with many internal frames, or a separate toplevel frame for each container request. An
 * implementation must have a no-arg constructor.
 */
public interface UIProvider {

  /**
   * The code for plugin container mode which specifies that the container must be open as long as
   * the session to the server exists. If the user attempts to close it, he will be asked for
   * confirmation and then the session will be closed along with the window.
   */
  public static final int ESSENTIAL_CONTAINER_MODE = 0;

  /**
   * The code for plugin container mode which specifies that the container may be hidden and then
   * shown again by the user as he wishes. The <code>UIProvider</code> will provide the user with UI
   * that allows him to hide/show the container.
   */
  public static final int HIDEABLE_CONTAINER_MODE = 1;

  /**
   * The code for plugin container mode which specifies that the container may be closed by the
   * user, but not reopened again. This should be mainly used for temporary containers.
   */
  public static final int CLOSEABLE_CONTAINER_MODE = 2;

  /**
   * The code for plugin container mode which specifies that the container is not to be managed by
   * the ui provider.
   */
  ...
  /**
   * This method is called at startup to ask the <code>UIProvider</code> to display its initial UI.
   * Presumably, the user can then use that UI to connect to the server.
   */
  void start();
  ...
  /**
   * Returns whether any UI is currently visible to the user. This is used by certain places in code
   * to quit Jin if no UI is visible. The proper way to solve this problem is to register shutdown
   * hooks, but that's not available under Java 1.1 (and Java 1.1 doesn't automatically shut down
   * when no UI is visible).
   */
  boolean isUiVisible();

  /**
   * This method is called to notify the <code>UIProvider</code> that Jin is being shut down. The
   * implementation is supposed to save any state it requires into preferences and close any UI
   * elements it owns.
   */
  void stop();
}
