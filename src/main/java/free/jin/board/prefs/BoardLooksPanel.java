...
public class BoardLooksPanel extends BoardModifyingPrefsPanel {
  ...
  /**
   * The panel containing all of the piece color selection controls. This is needed so that they can
   * be disabled when the selected piece painter is not a ColoredPiecePainter.
   */
  private Container pieceColorsPanel;

  /**
   * The panel containing all of the board color selection controls. This is needed so that they can
   * be disabled when the selected board painter is not a ColoredBoardPainter.
   */
  private Container boardColorsPanel;

  /**
   * Creates a new <code>BoardLooksPanel</code> for the specified <code>BoardManager</code> and with
  ...
  public BoardLooksPanel(BoardManager boardManager, JinBoard previewBoard) {
    ...
    pieceSets.addListSelectionListener(
        new ListSelectionListener() {
          ...
          public void valueChanged(ListSelectionEvent evt) {
            ...
            AWTUtilities.setContainerEnabled(
                pieceColorsPanel, painter instanceof ColoredPiecePainter);
            fireStateChanged();
          }
        });

    boardPatterns.addListSelectionListener(
        new ListSelectionListener() {
          @Override
          public void valueChanged(ListSelectionEvent evt) {
            ...
            AWTUtilities.setContainerEnabled(
                boardColorsPanel, painter instanceof ColoredBoardPainter);
            fireStateChanged();
          }
        });
    ...
    ChangeListener pieceColorChangeListener =
        new ChangeListener() {
          @Override
    ...
    ChangeListener boardColorChangeListener =
        new ChangeListener() {
          @Override
          public void stateChanged(ChangeEvent evt) {
    ...
    if (downloadExtras != null) downloadExtras.setAlignmentX(Component.CENTER_ALIGNMENT);
  ...
  private JComponent createBoardUI() {
    ...
    boardPanel.setBorder(
        BorderFactory.createCompoundBorder(
            i18n.createTitledBorder("boardPanel"), BorderFactory.createEmptyBorder(0, 10, 10, 10)));
