...
/** The panel which contains all the action buttons for a played game of type Game.MY_GAME. */
public class PlayedGameButtonPanel extends FixedJPanel implements ActionListener {
  ...
  /**
   * The offer state - when the user can merely offer an abort/adjourn/draw by pressing the
   * corresponding button.
   */
  protected static final int OFFER_STATE = 1;

  /**
   * The claim state - when the user can claim an abort/adjourn/draw by pressing the corresponding
   * button, without his opponent's consent.
   */
  protected static final int CLAIM_STATE = 2;

  /**
   * The accept state - when the abort/adjourn/draw has been offered by the opponent and the user
   * can accept it by pressing the corresponding button.
   */
  ...
  private static final Border CLAIM_STATE_BORDER =
      new LineBorder(CLAIM_STATE_COLOR, STATE_BORDER_SIZE);
  ...
  private static final Border ACCEPT_STATE_BORDER =
      new LineBorder(ACCEPT_STATE_COLOR, STATE_BORDER_SIZE);
  ...
  /** The game listener that gets notified of various game events we're interested in. */
  private GameListener gameListener =
      new GameAdapter() {
        ...
        public void gameEnded(GameEndEvent evt) {
          ...
          if (adjournButton != null) adjournButton.setEnabled(false);
          if (takeback1Button != null) takeback1Button.setEnabled(false);
          if (takebackNButton != null) takebackNButton.setEnabled(false);
  ...
  /**
   * Creates a new PlayedGameButtonPanel. It will be used by the given Plugin for the given Game.
   * The given parent Component determines over which component confirmation dialogs will be
   * displayed.
   */
  ...
  /**
   * Initializes this PlayedGameButtonPanel. This method calls delegates to {@link
   * #createComponents(Plugin, Game)} and {@link #addComponents(Plugin, Game)}
   */
  ...
  /** A list of the ply counts for all of the user's current outstanding takeback offers. */
  private final Vector userTakebacks = new Vector();

  /** A list of the ply counts for all of the opponent's current outstanding takeback offers. */
  ...
  protected void takebackOfferUpdate(boolean isOppsOffer, boolean isOffered, int plyCount) {
    ...
    if (isOffered) offers.addElement(new Integer(plyCount));
    else offers.removeElement(new Integer(plyCount));
    ...
    if (plies == 1) setTakeback1State(newState);
    else setTakebackNState(newState, newState == OFFER_STATE ? 2 : plies);
  }
  ...
  protected void createComponents(Plugin plugin, Game game) {
    ...
    takebackNButton =
        conn.isMultipleTakebackSupported() ? createButton("multipleTakebackButton") : null;
  }
  ...
  protected void setTakebackNState(int state, int plyCount) {
    if (takebackNButton == null) return;
    ...
    switch (state) {
      ...
      case OFFER_STATE:
        {
          takebackNButton.setToolTipText(
              i18n.getFormattedString("multipleTakebackButton.offerTooltip", plyCountArr));
      ...
      case CLAIM_STATE:
        {
          takebackNButton.setToolTipText(
              i18n.getFormattedString("multipleTakebackButton.claimTooltip", plyCountArr));
  ...
  /**
   * Sets the resign button's state to the specified value. This button may only be in the claim
   * state.
   */
  ...
  /**
   * ActionListener implementation. Executes the appropriate command depending on the button that
   * was pressed.
   */
  @Override
  public void actionPerformed(ActionEvent evt) {
    ...
    if (source == resignButton) {
      Object result =
          I18n.get(PlayedGameButtonPanel.class)
              .confirm(OptionPanel.OK, "resignConfirmation", parentComponent);
      if (result == OptionPanel.OK) conn.resign(game);
    } else if (source == drawButton) conn.requestDraw(game);
    else if (source == abortButton) conn.requestAbort(game);
    else if (source == adjournButton) conn.requestAdjourn(game);
    else if (source == takeback1Button) conn.requestTakeback(game);
