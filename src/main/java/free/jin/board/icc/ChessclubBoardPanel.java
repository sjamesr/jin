...
public class ChessclubBoardPanel extends BoardPanel
  ...
  /**
   * We set this to true when we're handling a circle/arrow adding event from the server to avoid
   * responding to the board's event when we add the circle/arrow to it.
   */
  private boolean handlingArrowCircleEvent = false;

  /**
   * Creates a new <code>ChessclubBoardPanel</code> with the given <code>BoardManager</code> and
   * <code>Game</code>.
   */
  ...
  /**
   * Overrides <code>createBoard(Game)</code> to return an instance of <code>ChessclubJBoard</code>.
   */
  @Override
