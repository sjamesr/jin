...
/**
 * An extension of <code>free.chess.JBoard</code> which implements certain Jin specific features.
 * This class adds the arrow/circle functionality but doesn't listen to the appropriate mouse events
 * to actually add arrows. This is done in the server specific subclasses. The "official" purpose of
 * this is to allow server specific gestures to generate arrows/circles. The "unofficial" reason is
 * so that it's possible to add arrows/circles programmatically on any server.
 */
public class JinBoard extends JBoard implements PaintHook {
  ...
  /**
   * Creates a new <code>JinBoard</code> with the given initial <code>Position</code>.
   */
  ...
  /**
   * Returns <code>true</code> if the arrow/circle adding functionality enabled (for the user),
  ...
  /**
   * Adds the given ArrowCircleListener to the list of listeners receiving notifications when a
   * circle or an arrow is added.
   */
  ...
  /**
   * Removes the given ArrowCircleListener from the list of listeners receiving notifications when a
   * circle or an arrow is added.
   */
  ...
  /**
   * Notifies all registered ArrowCircleListeners that an arrow has been removed.
   */
  ...
  /**
   * Notifies all registered ArrowCircleListeners that a circle has been removed.
   */
  ...
  public void paint(Component component, Graphics g) {
    if (component != this) throw new IllegalArgumentException("Can only paint on this");
  ...
  public boolean areArrowsAt(Square from, Square to) {
    for (int i = 0; i < arrows.size(); i++) {
      Arrow arrow = (Arrow) arrows.elementAt(i);
      if (arrow.getFrom().equals(from) && arrow.getTo().equals(to)) return true;
    }
  ...
  public void removeAllArrows() {
    ...
    for (int i = arrows.size() - 1; i >= 0; i--) {
      ...
      if (rect == null) rect = arrowRect;
      else rect = rect.union(arrowRect);
    }

    if (rect != null) repaint(rect);
  }
  ...
  /**
   * Removes the specified circle (or circles, if there's more than one) from the board.
   */
  ...
  public boolean areCirclesAt(Square square) {
    for (int i = 0; i < circles.size(); i++) {
      Circle circle = (Circle) circles.elementAt(i);
      if (circle.getSquare().equals(square)) return true;
    }
  ...
  public void removeAllCircles() {
    ...
    for (int i = circles.size() - 1; i >= 0; i--) {
      ...
      if (rect == null) rect = circleRect;
      else rect = rect.union(circleRect);
    }
