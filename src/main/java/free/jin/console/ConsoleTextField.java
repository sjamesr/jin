...
public class ConsoleTextField extends FixedJTextField {
  ...
  /**
   * The index of the history entry currently displayed, -1 if the string currently displayed is not
   * in the history.
   */
  private int currentHistoryIndex = -1;

  /**
   * The string that keeps the text the user typed in before he started traversing the history
   * buffer.
   */
  ...
  private final Action sendAction =
      new AbstractAction(I18n.get(ConsoleTextField.class).getString("sendAction.name")) {
        ...
        public void actionPerformed(ActionEvent evt) {
          ...
          long modifiers = 0;
          if (isShiftDown) modifiers |= Command.BLANKED_MASK;
          if (isControlDown) modifiers |= Command.SPECIAL_MASK;
  ...
  protected void initFromProperties() {
    ...
    if (tellNextTellerKeyStroke != null) unregisterKeyboardAction(tellNextTellerKeyStroke);
  ...
  protected void processKeyEvent(KeyEvent evt) {
    ...
    if (evt.getID() == KeyEvent.KEY_PRESSED) {
      if (((keyCode == KeyEvent.VK_INSERT) && isShiftDown)
          || // The shift-insert keybinding
  ...
  protected void processComponentKeyEvent(KeyEvent evt) {
    ...
    if (evt.getID() == KeyEvent.KEY_PRESSED) {
      switch (evt.getKeyCode()) {
        case KeyEvent.VK_ENTER:
          ActionEvent actionEvent =
              new ActionEvent(this, ActionEvent.ACTION_PERFORMED, "send", evt.getModifiers());
        ...
        case KeyEvent.VK_UP:
          if (evt.getModifiers() == 0) {
            if (currentHistoryIndex == -1) typedInString = getText();
        ...
        case KeyEvent.VK_DOWN:
          if (evt.getModifiers() == 0) {
            ...
            if (newHistoryIndex == -1) {
              setText(typedInString);
              currentHistoryIndex = -1;
            } else setText((String) history.elementAt(currentHistoryIndex));
          }
          break;
        case KeyEvent.VK_R:
          if (isControlDown) { // Watauba feature :-)
            ...
            if (selectedText != null) {
              ...
              setText(
                  curText.substring(0, selectionStart)
                      + reversedSelectedText
      ...
      if (evt.getKeyChar() != FixUtils.CHAR_UNDEFINED) tellerIndex = -1;
    }
  }
  ...
  /**
   * Sets the textfield to be ready to send a tell to the given player. The default implementation
   * does nothing.
   */
  protected void setTellPersonState(ServerUser playerName) {}

  /**
   * The ActionListener setting the current console textfield text to something that will send a
   * tell to the last player who told us something.
   */
  private class TellLastTellerAction implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      if (teller != null) setTellPersonState(teller);
    }
  }

  /**
   * The ActionListener setting the current console textfield text to something that will send a
   * tell to the next player who told us something.
   */
    ...
    ActionListener popupListener =
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            if (src == cut) cut();
            else if (src == copy) copy();
            else if (src == paste) paste();
          }
        };
  ...
  /**
   * Configures the popup prior to showing it - enables or disabled certain menu items of the popup,
