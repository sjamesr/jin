...
public class ChessclubConsoleManager extends IcsConsoleManager {
  ...
  protected String getDefaultTextForChat(ChatEvent evt, String encoding) {
    ...
    if ("qtell".equals(type)) return ChessclubConsoleManager.parseQTell(evt);
    else if ("channel-qtell".equals(type)) return ChessclubConsoleManager.parseChannelQTell(evt);
  ...
  protected ConsoleDesignation createGeneralChatConsoleDesignation(boolean isCloseable) {
    ...
    else return new IccGeneralChatConsoleDesignation(getConn(), getEncoding(), isCloseable);
    //    if (isWcl()){
  ...
  protected IcsCustomConsoleDesignation loadCustomConsoleDesignation(
    ...
    return new ChessclubCustomConsoleDesignation(
  ...
  protected SortedMap createChannels() {
    ...
    Comparator channelsComparator =
        new Comparator() {
          ...
          public int compare(Object arg0, Object arg1) {
            ...
            if (position1 == null) {
              if (position2 == null) return i1.intValue() - i2.intValue();
              else return 1;
            } else {
              if (position2 == null) return -1;
              else return position1.intValue() - position2.intValue();
            }
          }
        };
    ...
    if (isWcl) {
      ...
      for (int i = 0; i < 400; i++) channels.put(new Integer(i), new IccChannel(i));
    }
  ...
  /**
   * Returns a string that should be displayed for the given ChatEvent when the ChatEvent contains a
   * qtell.
   */
  ...
  /**
   * ICC no longer allows users to do almost anything, so we conveniently display the help channel
   * for guest users on-login.
   */
  @Override
  public void loginSucceeded(Connection conn) {
    super.loginSucceeded(conn);

    if (getUser().isGuest()) activateHelpConsole(MAIN_CONTAINER_ID);
  }
}
