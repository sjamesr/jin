...
public abstract class IcsCustomConsoleDesignation extends CustomConsoleDesignation {
  ...
  public IcsCustomConsoleDesignation(
    ...
    addAccepted(
        "announcement", null, ANY_SENDER); // Almost all consoles should display announcements

    if (isIncludeShouts) addShouts();
  ...
  protected void addShouts() {
    ...
    addCommandType(
        new AbstractCommandType(i18n.getString("shout.commandName")) {
          @Override
    ...
    addCommandType(
        new AbstractCommandType(i18n.getString("ishout.commandName")) {
          @Override
          protected void send(String userText) {
  ...
  protected boolean accept(JinEvent evt) {
    ...
    if (messageRegex != null) {
      String message = null;
      if (evt instanceof PlainTextEvent) message = ((PlainTextEvent) evt).getText();
      else if (evt instanceof ChatEvent) message = ((ChatEvent) evt).getMessage();

      if (messageRegex.matcher(message).matches()) return true;
    }
    ...
    public TellChannelCommandType(IcsChannel channel) {
      super(
          I18n.get(TellChannelCommandType.class)
              .getFormattedString("name", new Object[] {String.valueOf(channel.getId())}));
