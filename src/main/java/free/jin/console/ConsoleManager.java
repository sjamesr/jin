 ...
 * and closing the various consoles used by the user. The class is abstract because some of the
 * functionality is delegated to server-specific classes.
 */
public abstract class ConsoleManager extends Plugin
  ...
  private void createConsoles() {
    ...
    for (Iterator i = tabbedPanes.entrySet().iterator(); i.hasNext(); ) {
      ...
      int defaultSelectedIndex =
          prefs.getInt("containers." + containerId + ".defaultSelectedTab", 0);
      int selectedIndex =
          prefs.getInt("containers." + containerId + ".selectedTab", defaultSelectedIndex);
  ...
  private ConsoleDesignation loadConsoleDesignation(String prefsPrefix) {
    ...
    if ("system".equals(type)) {
      ...
      List channels =
          channelsPref == null ? Collections.EMPTY_LIST : parseConsoleChannelsPref(channelsPref);
      ...
      return loadCustomConsoleDesignation(
          prefsPrefix, title, encoding, channels, messageRegexPattern);
    } else throw new IllegalArgumentException("Unrecognized designation type: " + type);
  }

  /**
   * Creates the system console designation. This method is meant to be implemented by a
   * server-specific console manager. Note that the returned designation is treated specially - it
   * is only passed events which were not already handled by any of the other consoles.
   */
  protected abstract ConsoleDesignation createSystemConsoleDesignation();

  /**
   * Creates the help console designation. This method is meant to be implemented by a
   * server-specific console manager.
   */
  protected abstract ConsoleDesignation createHelpConsoleDesignation(boolean isCloseable);

  /**
   * Creates the general chat console designation. This method is meant to be implemented by a
   * server-specific console manager.
   */
  ...
  private void createUI() {
    ...
    Image iconImage =
        iconImageURL == null ? null : Toolkit.getDefaultToolkit().getImage(iconImageURL);
    ...
    if (iconImage != null) mainConsoleContainer.setIcon(iconImage);
    ...
    for (int i = 0; i < customContainersCount; i++) {
      ...
      if (iconImage != null) container.setIcon(iconImage);
  ...
  private void obtainFocus(PluginUIContainer container) {
    ...
    Console console;
    if (consolesInContainer.size() == 1) console = (Console) consolesInContainer.get(0);
    else {
  ...
  public Console getConsole(ConsoleDesignation designation) {
    for (int i = 0; i < consoles.size(); i++) {
      ...
      if (console.getDesignation().equals(designation)) return console;
    }
  ...
  /**
   * Adds a console with the specified designation, possibly making it the active console, to the
   * console container with the specified ID. If a console with the specified designation already
   * exists, and <code>makeActive</code> is <code>true</code>, it is merely made active (otherwise,
   * no action is taken).
   */
  public void addConsole(
      final ConsoleDesignation designation, String containerId, boolean makeActive) {
    ...
    if (console == null) {
      ...
      final Tab tab =
          new Tab(console, designation.getName(), null, designation.isConsoleCloseable());
      ...
      designation.addPropertyChangeListener(
          new PropertyChangeListener() {
            @Override
            public void propertyChange(PropertyChangeEvent evt) {
              String propertyName = evt.getPropertyName();
              if ("name".equals(propertyName)) tab.setTitle(designation.getName());
  ...
  /**
   * Makes the specified console selected inside the container with the specified ID, making it the
   * one displayed in the tabbed pane.
   */
  ...
  public String getActiveContainerId() {
    for (Iterator i = consoleContainers.entrySet().iterator(); i.hasNext(); ) {
      ...
      if (container.isActive()) return (String) entry.getKey();
    }
  ...
  /**
   * Adds a closeable console for chatting with the specified user, possibly making it the active
   * console, to the container with the specified ID. If one already exists, it is merely activated,
   * if <code>makeActive</code> is <code>true</code> (otherwise, no action is taken).
   *
   * @returns the newly created personal chat designation.
   */
  public ConsoleDesignation addPersonalChatConsole(
      ServerUser user, String containerId, boolean makeActive) {
  ...
  /**
   * Adds a new temporary console which will send the specified commands to the server and display
   * their output, to the container with the specified ID.
   */
  public void addTemporaryConsole(String[] commands, String containerId) {
    ...
    String consoleName =
        getI18n()
            .getFormattedString("temporaryConsole.title", new Object[] {String.valueOf(index)});
    ConsoleDesignation designation =
        new TemporaryConsoleDesignation(getConn(), consoleName, getEncoding(), true, commands);
    addConsole(designation, containerId, true);
  }

  /**
   * Adds a closeable console for getting help on the server, into the container with the specified
   * ID. If such a console already exists, it is merely activated.
   */
  public void activateHelpConsole(String containerId) {
    Console helpConsole = null;

    if (helpConsoleDesignation == null) helpConsoleDesignation = createHelpConsoleDesignation(true);
    else helpConsole = getConsole(helpConsoleDesignation);
  ...
  /**
   * Rereads the plugin/user properties and changes settings accordingly. This method should be
   * called when the user changes the preferences.
   */
  ...
  protected void eventForConsoleReceived(JinEvent evt) {
    ...
    for (int i = 0; i < consoles.size(); i++) {
      ...
      if (designation != systemConsoleDesignation) handled |= console.getDesignation().receive(evt);
    }

    // We open a new personal chat console only if the event wasn't already handled.
    if (!handled
        && (evt instanceof ChatEvent)
      ...
      if (containerId == null) containerId = MAIN_CONTAINER_ID;

      ConsoleDesignation designation =
          addPersonalChatConsole(chatEvent.getSender(), containerId, false);
    ...
    if (!isPaused) {
      ...
      for (int i = 0; i < size; i++) {
        try {
          ...
          if (evt instanceof PlainTextEvent) plainTextReceived((PlainTextEvent) evt);
          else if (evt instanceof ChatEvent) chatMessageReceived((ChatEvent) evt);
        } catch (Exception e) {
          e.printStackTrace();
        }
      }
  ...
  /**
   * Returns the default text to be added to a console when receiving the specified chat event
   * (decoded to the specified encoding).
   */
  ...
  public void connectionAttempted(Connection conn, String hostname, int port) {
    ...
    String message =
        getI18n()
            .getFormattedString(
                "tryingToConnectMessage", new Object[] {hostname, String.valueOf(port)});
  ...
  public void loginSucceeded(Connection conn) {
    PluginUIContainer mainConsoleContainer =
        (PluginUIContainer) consoleContainers.get(MAIN_CONTAINER_ID);
    String title =
        getI18n()
            .getFormattedString(
                "mainConsole.title",
  ...
  protected void gameStarted(GameEvent evt) {
    if (getPrefs().getBool("newConsoleOnGameStart", false)) {
      ...
      boolean makeSelectedAndActive =
          !((evt.getGame().getGameType() == Game.MY_GAME) && !evt.getGame().isPlayed());
      ...
      if (makeSelectedAndActive) makeActive(containerId);
    }
  }

  /**
   * Returns the channels/rooms available on the server we're logged on to (a map from channel IDs
   * to {@link Channel} objects). The order of the map is the order in which the channels should be
   * normally displayed to the user.
   */
  public SortedMap getChannels() {
    if (channels == null) channels = createChannels();

    return Collections.unmodifiableSortedMap(channels);
  }

  /**
   * Returns the set of channels/rooms for the server we're connecting to (a map from channel IDs to
   * {@link Channel} objects). This method is meant to be implemented by server-specific classes.
   */
  ...
  public void saveState() {
    ...
    for (Iterator i = tabbedPanes.entrySet().iterator(); i.hasNext(); ) {
      ...
      prefs.remove(key);
      else prefs.setInt(key, selectedIndex);
    }
  ...
  /**
   * Overrides <code>hasPreverencesUI</code> to return whether the plugin will display a preferences
   * UI (the setting is taken from the
   *
   * <pre>"preferences.show"</pre>
   *
   * property.
   */
  @Override
