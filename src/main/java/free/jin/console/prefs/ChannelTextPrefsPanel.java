 ...
 * panel from properties. The channels category panel allows choosing text styles for numbered
 * channels, such as ones available on chessclub.com, freechess.org and similar servers. To add such
 * a panel you must specify that the category panel is custom, give it the an id ending with the
 * string
 *
 ...
 * <pre>"channels-panel"</pre>
 *
 * part of the property name can be replaced with any string ending with
 *
 * <pre>"channels-panel"</pre>
 *
...
public class ChannelTextPrefsPanel extends TextPrefsPanel {
  ...
  /**
   * Creates a new <code>ChannelTextPrefsPanel</code> for the specified <code>ConsoleManager</code>.
   */
  ...
  /**
   * Overrides <code>createCustomCategoryPanel</code> to create <code>ChannelCategoryPanels</code>.
   */
  @Override
  protected CategoryPanel createCustomCategoryPanel(String id) {
    if (id.endsWith("channels-panel")) {
      ...
      ChannelsCategoryPanel channelCategoryPanel =
          createChannelsCategoryPanel(categoryName, categoryIDs, channelsCount);
  ...
  private ChannelsCategoryPanel createChannelsCategoryPanel(
    ...
    TextStyleChooserPanel textStyleChooser =
        new TextStyleChooserPanel(font, foreground, background, antialias, false, false);

    return new ChannelsCategoryPanel(categoryName, textStyleChooser, categoryIDs, channelsCount);
  }

  /**
   * An extension of <code>TextPrefsPanel.CategoryPanel</code> which allows selecting the text style
   * of numbered channels. It pretends to be a CategoryPanel for different categories depending on
   * the currently specified (by the user) channel.
   */
  private class ChannelsCategoryPanel extends CategoryPanel {
    ...
    /**
     * Creates a new ChannelsCategoryPanel with the specified category name, category ids,
     * TextStyleChooserPanel and amount of channels available on the server.
     */
    public ChannelsCategoryPanel(
        String categoryName,
        TextStyleChooserPanel textStyleChooser,
        String[] categoryIDs,
        int channelsCount) {
      ...
      channelNumberField =
          new free.workarounds.FixedJTextField(
              new IntegerStrictPlainDocument(0, channelsCount),
              "",
    ...
    /**
     * Sets the TextStyleChooserPanel to display the settings of the currently selected channel. If
     * the channel string is an empty string, the panel will show the default channel settings.
     */
    ...
    /**
     * Returns the currently displayed channel (as a string) or an empty string if the default
     * channel settings are displayed.
     */
    ...
    /**
     * Overrides getCategories() to pretend to be editing different categories depending on the
     * currently specified channel.
     */
    @Override
    public String[] getCategories() {
      ...
      if (!"".equals(channel))
        for (int i = 0; i < categories.length; i++) categories[i] = categories[i] + "." + channel;
