...
public class TextPrefsPanel extends PreferencesPanel {
  ...
  /**
   * Updates the properties from the settings of the specified CategoryPanel. This method is called
   * whenever the settings on the current CategoryPanel change.
   */
  protected void updatePropertiesFrom(CategoryPanel categoryPanel) {
    ...
    if (textStyleChooser == null)
      throw new IllegalStateException(
          "Updating preferences from a " + "nondisplayed category panel");
    ...
    if (categoryPanel == defaultSettingsPanel) {
      ...
      if (textStyleChooser.isAntialiasingSelectionEnabled())
        prefs.setBool(
            "output-text.antialias", defaultSettingsPanel.getTextStyleChooser().isAntialias());
    }
  ...
  /**
   * Updates all the CategoryPanels and their TextStyleChooserPanels from the properties. This
   * method is called whenever the properties change which may require updating some of the category
   * panels.
   */
  protected void updatePanels() {
    selectionColorButton.setColor(
        prefs.getColor("output-selection", UIManager.getColor("textHighlight")));
    selectedColorButton.setColor(
        prefs.getColor("output-selected", UIManager.getColor("textHighlightText")));
    ...
    for (int i = 0; i < categoryPanels.size(); i++) {
      ...
      TextStyleChooserPanel textStyleChooser = panel.getTextStyleChooser();
      if (textStyleChooser == null) continue;
  ...
  private void createDefaultSettingsPanel() {
    ...
    TextStyleChooserPanel defaultSettingsChooserPanel =
        new TextStyleChooserPanel(
            font, foreground, background, antialiasingValue, true, antialiasingSupported);
    ...
    defaultSettingsPanel =
        new CategoryPanel(
            I18n.get(TextPrefsPanel.class).getString("defaultTextCategoryName"),
            defaultSettingsChooserPanel,
            new String[] {""});
   ...
   * method in the subclass and check the ID when invoked. When the given ID matches the one you
   * specified in the properties, return your custom <code>CategoryPanel</code>. If the ID does not
   * match, simply invoke the superclass' method and return whatever it returns.
   */
  ...
  /**
   * Creates and returns the <code>ColorChooser</code> used for choosing the color of the selection.
   */
  protected ColorChooser createSelectionColorButton() {
    ColorChooser button =
        I18n.get(TextPrefsPanel.class).createColorChooser("selectionColorChooser");
  ...
  /**
   * Creates and returns the <code>ColorChooser</code> used for choosing the color of selected text.
   */
  protected ColorChooser createSelectedColorButton() {
    ColorChooser button =
        I18n.get(TextPrefsPanel.class).createColorChooser("selectedTextColorChooser");
  ...
  protected Font getCategoryFont(String categoryName) {
    ...
    int fontSize =
        ((Integer) prefs.lookup("font-size." + categoryName, new Integer(14))).intValue();
    int fontStyle = 0;
  ...
  /**
   * Sets the value of the given property type in the given category to the specified value. If the
   * property is marked as unmodifiable, the call is silently ignored.
   */
  protected void setProperty(String categoryName, String propertyType, Object propertyValue) {
    String propertyName =
        (categoryName.equals("") ? propertyType : propertyType + "." + categoryName);
    if (prefs.getBool(propertyName + ".unmodifiable", false)) return;
  ...
  protected class CategoryPanel extends JPanel {
    ...
    /**
     * The IDs (short names) of categories that need to be updated when the settings of the text
     * style chooser change.
     */
    private final String[] categories;

    /**
     * Creates a new CategoryPanel with the specified long category name, the TextStyleChooserPanel
     * in this panel and a list of category IDs (short names) that need to be modified when the
     * TextStyleChooserPanel's settings change. The "main" category, which is the category used to
     * display the settings is the first element (at index zero) in the categories array.
     */
    public CategoryPanel(
        String categoryName, TextStyleChooserPanel textStyleChooser, String[] categories) {
    ...
    /**
     * Creates a new CategoryPanel with the specified long category name, a list of category IDs
     * (short names) that need to be modified when the TextStyleChooserPanel's settings change and a
     * lazily created TextStyleChooserPanel. The "main" category, which is the category used to
     * display the settings is the first element (at index zero) in the categories array.
     */
    ...
    /**
     * Returns the TextStyleChooser in this panel, or <code>null</code> if it hasn't been created
     * yet.
     */
    ...
    /**
     * Returns the IDs (short names) of categories that need to be updated when the text style
     * chooser's settings change.
     */
