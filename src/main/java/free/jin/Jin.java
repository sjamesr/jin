...
public class Jin {
  ...
  static {
    try {
      appProps =
          IOUtilities.loadPropertiesAndClose(Resources.getResource("app.props").openStream());
    } catch (IOException e) {
      e.printStackTrace();
  ...
  /**
   * Cache of loaded resources. Maps a pair of (resourceType, plugin) to a pair whose first value is
   * a map from resourceId to the resource and whose second value is a Boolean specifying whether
   * the map consists of all the resources of that type for that plugin (it may also be partially
   * loaded).
   */
  ...
  private Jin(JinContext context) {
    ...
    for (int i = 0; i < usersArr.length; i++) users.addElement(usersArr[i]);
  ...
  private void restoreLookAndFeel() {
    ...
    Preferences prefs =
        getCustomizingPrefs() == null
            ? getPrefs()
            : Preferences.createBackedUp(getPrefs(), getCustomizingPrefs());

    String lfClassName =
        (String) prefs.lookup("lookAndFeel.classname." + PlatformUtils.getOSName(), null);

    try {
      if (lfClassName != null) UIManager.setLookAndFeel(lfClassName);
      else UIManager.setLookAndFeel(defaultLnf);
    } catch (Exception e) {
      if (lfClassName != null)
        JOptionPane.showMessageDialog(
            null,
            "Unable to use the specified look and feel: \n"
                + TextUtilities.breakIntoLines(e.getMessage(), 60),
            "Jin Error",
  ...
  /**
   * Returns the value of the specified application property, or the specified default value if none
   * exists.
   */
  ...
  /**
   * Returns the value of the specified parameter, as passed to the application when it was run.
   * Returns <code>null</code> if no parameter with the specified name was passed.
   */
  ...
  /**
   * Returns a list of <code>ActionInfo</code> objects describing the standalone actions for the
   * specified server.
   */
  ...
  /**
   * Returns a list of <code>PluginInfo</code> objects describing the plugins for the specified
   * server.
   */
  ...
  /**
   * Loads and returns the resources of the specified type for the specified plugin. The returned
   * map is from resource IDs to <code>Resource</code>s. Resources are typically used when there is
   * a need to allow the user (or some other 3rd party) to add his own customizations to Jin (or a
   * plugin). For example, this mechanism is used for loading piece sets and boards by the board
   * manager plugin. A <code>JinContext</code> implementation may then look for piece set "packs" in
   * some predefined directories, allowing the user to add/remove piece sets simply by
  ...
  public Map getResources(String resourceType, Plugin plugin) {
    ...
    } else resourceMap = (Map) value.getFirst();
  ...
  /**
   * Returns the list of known users (accounts on various servers). This list will be updated as
   * users are added or removed, so you may register listeners with it if you wish to be notified.
   * The list does not include guest users.
   */
  ...
  /**
   * Returns whether the specified User represents a "known" account, that is, it appears in the
   * list returned by <code>getUsers</code>.
   */
  public boolean isKnownUser(User user) {
    for (int i = 0; i < users.getSize(); i++) if (users.getElementAt(i).equals(user)) return true;
  ...
  public Server getServerById(String id) {
    for (Server server : context.getServers()) if (server.getId().equals(id)) return server;
  ...
  public List getUsers(Server server) {
    ...
    for (int i = 0; i < users.getSize(); i++) {
      ...
      if (user.getServer() == server) serverUsers.add(user);
    }
    return serverUsers;
  }

  /**
   * Returns the user with the specified username on the specified server or <code>null</code> if no
   * such user exists. Doesn't work for guest users.
   */
  public User getUser(Server server, String username) {
    for (int i = 0; i < users.getSize(); i++) {
      User user = (User) users.getElementAt(i);
      if ((user.getServer() == server)
          && server.getUsernamePolicy().isSame(username, user.getUsername())) return user;
    }
  ...
  /**
   * Returns whether the context in which Jin is running allows the end-user to extend Jin by
   * running extra plugins, actions, resources etc.
   */
  ...
  /**
   * If no UI is currently visible, quits the application. This was originally needed because in
   * Java 1.1 the AWT thread didn't quit when all UI was closed. It is still needed, however,
   * because nowadays (1.5 on Mac OS X, at least) there's an audio related non-daemon thread, which
   * also doesn't quit.
   */
  public void quitIfNoUiVisible() {
    if (!getUIProvider().isUiVisible()) quit(false);
  }

  /**
   * Quits the application, possibly asking the user to confirm quitting first. This method doesn't
   * necessarily return.
   */
  public void quit(boolean askToConfirm) {
    ...
    Object result =
        askToConfirm
            ? i18n.confirm(OptionPanel.OK, "quitConfirmationDialog", new Object[] {getAppName()})
            : OptionPanel.OK;
