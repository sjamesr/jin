...
/** A plugin which displays the various available actions and lets the user run them. */
public class ActionsPlugin extends Plugin {
  ...
  /** The order of actions - a list of their IDs; <code>null</code> if the order doesn't matter. */
  private List actionsOrder;

  /** The actions to include - a list of their IDs; <code>null</code> if to include everything. */
  ...
  public void start() {
    ...
    else actionsIncludeFilter = null;

    buttonContainer = createButtonContainer();
    buttonContainer.addPluginUIListener(
        new PluginUIAdapter() {
          @Override
  ...
  /**
   * Filters the specified list of actions according to our filter setting and returns the resulting
   * list.
   */
  private List filter(List actions) {
    if (actionsIncludeFilter == null) return actions;

    List filteredActions = new LinkedList();
    for (Iterator i = actions.iterator(); i.hasNext(); ) {
      JinAction action = (JinAction) i.next();
      if (actionsIncludeFilter.contains(action.getId())) filteredActions.add(action);
    }
  ...
  /**
   * Sorts the specified actions according to our "order" setting and returns the resulting list.
   */
  private List sort(List actions) {
    if (actionsOrder == null) return actions;
    ...
    Collections.sort(
        ...
        new Comparator() {
          ...
          public int compare(Object arg0, Object arg1) {
            ...
            if (index1 == -1) return (index2 == -1) ? 0 : 1;
            else if (index2 == -1) return (index1 == -1) ? 0 : -1;
  ...
  private void updateActionButtons() {
    ...
    if (buttonContainer.isVisible()) buttonContainer.pack();
  }
}
