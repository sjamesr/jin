 ...
 * action. An action can be created in two ways - either it is created by the Jin framework as a
 * standalone action or it can be created and exported by a plugin. Subclasses which implement
 * standalone actions must have a no-arg constructor.
 */
public abstract class JinAction extends AbstractAction {
  ...
  /**
   * Creates a new <code>JinAction</code> with its name and short descriptions retrieved from its
   * I18n using {@link I18n#initAction(Action, String)} and its icon image loaded from a resource
   * named <code>getId() + ".png"</code> (if such a resource exists).
   */
  public JinAction() {
  ...
  /**
   * Returns whether the specified context is supported by this action. The default implementation
    ...
    if (prefs == null) {
      ...
      prefs =
          Preferences.createBackedUp(
              Preferences.createWrapped(userPrefs, "action." + getId() + "."), actionPrefs);
    }
  ...
  /**
   * Returns the action's id. This will not be displayed to the user and must be unique between all
   * actions.
   */
