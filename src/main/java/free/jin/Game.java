 ...
 * features that are not supported by some servers - it's up to the JinConnection implementation to
 * emulate them as closely as possible. If it's genuinely impossibly to implement the feature, the
 * implementation should provide a reasonable blank value. Server specific subclasses should add
 * methods for retrieving all the information provided by the server.
 */
public class Game {
  ...
  /**
   * The constant for when the game result is not yet known because the game hasn't finished yet.
   */
  ...
  /**
   * Sets the amount of plies made from the beginning of the actual game to the initial position as
   * specified by this <code>Game</code> object.
   */
  public void setPliesSinceStart(int plies) {
    if (plies < 0) throw new IllegalArgumentException("plies may not be negative");

    props.setIntegerProperty("pliesSinceStart", plies);
  }

  /**
   * Returns the amount of half-moves made from the beginning of the actual game to the initial
   * position as specified by this <code>Game</code> object.
   */
  ...
  /**
   * Returns an object identifying the game. This object, when converted to a String (via the
   * toString() method) could be displayed to the user.
   */
  public Object getID() {
  ...
  /**
   * Returns a String identifying the rating category of the game - this should be displayed to the
   * user.
   */
  ...
  /**
   * Returns the player for whom the user plays. This is only relevant if the game is of type
   * MY_GAME and is a played game, in all other cases, this method returns null.
   */
  public Player getUserPlayer() {
    if ((getGameType() == MY_GAME) && isPlayed()) return (Player) props.getProperty("userPlayer");
    else return null;
  }
  ...
  public boolean isUserAllowedToMovePieces(Player player) {
    ...
    if (gameType == OBSERVED_GAME) return false;
    else if (gameType == ISOLATED_BOARD) return false;
    else // MY_GAME
  ...
  public int getResultCode() {
    ...
    if (result == null) return GAME_IN_PROGRESS;
    return result.intValue();
  }

  /**
   * Returns the reason code of the game end. Possible values are one of the reason codes defined in
   * this class. May only be invoked if the result code is not <code>GAME_IN_PROGRESS</code>. See
   * also {@link #getGameEndActor()}.
   */
  ...
  /**
   * Returns a string describing the game end reason, which is suitable to be displayed to the user.
   */
  public String getGameEndReasonDescription() {
    ...
    switch (reasonCode) {
      case RESIGNS:
        ...
        break;
      case CHECKMATED:
        reasonKey = "checkmated";
        break;
      case TIME_FORFEITS:
        reasonKey = "timeForfeits";
        break;
      case ADJUDICATED:
        reasonKey = "adjudicated";
        break;
      case DISCONNECTED:
        reasonKey = "disconnected";
        break;
      case PARTNER_RESIGNS:
        reasonKey = "partnerResigns";
        break;
      case PARTNER_CHECKMATED:
        reasonKey = "partnerCheckmated";
        break;
      case PARTNER_TIME_FORFEITS:
        reasonKey = "partnerTimeForfeits";
        break;
      case PARTNER_DISCONNECTED:
        reasonKey = "partnerDisconnected";
        break;
      case DRAW_AGREEMENT:
        reasonKey = "drawAgreement";
        break;
      case STALEMATE:
        reasonKey = "stalemate";
        break;
      case REPETITION:
        reasonKey = "repetition";
        break;
      case FIFTY_MOVE_RULE:
        reasonKey = "50move";
        break;
      case OUT_OF_TIME_AND_OPP_HAS_NO_MATERIAL_TO_MATE:
        reasonKey = "outOfTimeAndOppHasNoMaterial";
        break;
      case BOTH_NO_MATERIAL_TO_MATE:
        reasonKey = "bothNoMaterial";
        break;
      case BOTH_OUT_OF_TIME:
        reasonKey = "bothOutOfTime";
        break;
      case PARTNER_DRAW_AGREEMENT:
        reasonKey = "partnerDrawAgreement";
        break;
      case PARTNER_BOTH_OUT_OF_TIME:
        reasonKey = "partnerBothOutOfTime";
        break;
      case ADJOURNED_AGREEMENT:
        reasonKey = "adjournedAgreement";
        break;
      case ADJOURNED_DISCONNECTED:
        reasonKey = "adjournedDisconnected";
        break;
      case ADJOURNED_COURTESY:
        reasonKey = "adjournedCourtesy";
        break;
      case ADJOURNED_ADMIN:
        reasonKey = "adjournedAdmin";
        break;
      case ABORTED_AGREEMENT:
        reasonKey = "abortedAgreement";
        break;
      case ABORTED_DISCONNECTED:
        reasonKey = "abortedDisconnected";
        break;
      case ABORTED_COURTESY:
        reasonKey = "abortedCourtesy";
        break;
      case ABORTED_ADMIN:
        reasonKey = "abortedAdmin";
        break;
      case ABORTED_TOO_SHORT:
        reasonKey = "abortedTooShort";
        break;
      case ABORTED_PARTNER_DISCONNECTED:
        reasonKey = "abortPartnerDisconnected";
        break;
      default:
        reasonKey = null;
    }
    ...
    String actorKey;
    if (actor == null) actorKey = null;
    else if (actor.isWhite()) actorKey = "white";
    else actorKey = "black";
    ...
    for (int i = keys.length - 1; i >= 1; i--) {
      ...
      if (description != null) return i18n.getFormattedString(keys[i], args);
    }
    ...
    if (getGameType() == Game.MY_GAME) {
      if (isPlayed())
        return i18n.getFormattedString(
            "playingShortDescription", new Object[] {getUserPlayer().isWhite() ? black : white});
      else if (getWhiteName().equals(getBlackName()))
        return i18n.getFormattedString(
            "examinationShortDescription", new Object[] {white, black, resultString});
      else
        return i18n.getFormattedString(
            "examiningShortDescription", new Object[] {white, black, resultString});
    } else
      return i18n.getFormattedString(
  ...
  public String toString() {
    return "#"
        ...
        + " "
        + getTimeControl().getLocalizedShortDescription()
        + " "
        + getVariant().getName()
        + " "
        + getWhiteName()
