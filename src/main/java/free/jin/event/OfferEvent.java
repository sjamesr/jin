...
/**
 * The event fired when an offer, such as a draw, abort or adjourn offer, is made or withdrawn in a
 * game.
 */
public class OfferEvent extends GameEvent {
  ...
  public OfferEvent(
    ...
    if (takebackCount <= 0) throw new IllegalArgumentException("takebackCount must be positive");
  ...
  /**
   * Returns <code>true</code> if the offer is relevant (that is, the opponent can accept it),
  ...
  /**
   * Returns the amount of plies offered to take back. Throws an exception if the offer is not a
   * takeback offer.
   */
  public int getTakebackCount() {
    if (offerId != TAKEBACK_OFFER) throw new IllegalStateException("The offer is not takeback");
  ...
  public String toString() {
    ...
    switch (getOfferId()) {
      case DRAW_OFFER:
        buf.append("DRAW_OFFER,");
        break;
      case ABORT_OFFER:
        buf.append("ABORT_OFFER,");
        break;
      case ADJOURN_OFFER:
        buf.append("ADJOURN_OFFER,");
        break;
      case TAKEBACK_OFFER:
        buf.append("TAKEBACK_OFFER(" + getTakebackCount() + "), ");
        break;
    }

    buf.append(getPlayer() + ", ");

    if (isOffered()) buf.append("offered]");
    else buf.append("not offered]");
