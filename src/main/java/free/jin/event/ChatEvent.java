...
/**
 * The event that is sent to ChatListeners when a chat related message arrives from the server. The
...
public class ChatEvent extends JinEvent {
  ...
  /**
   * The constant for chat belonging to a certain "room" or "channel" which players can choose to be
   * in.
   */
  ...
  /**
   * The type of the ChatEvent. This is something server specific and thus should only be handled by
  ...
  /**
   * The forum on which the message was sent. The actual type of the forum is server-dependent, and
   * may even be <code>null</code>, but for events of category {@link #GAME_CHAT_CATEGORY}, this
   * should be the game object.
   */
  private final Object forum;

  /**
   * Creates a new ChatEvent with the given type, category, sender, sender titles, sender rating (-1
   * if unknown), message and forum. Note that the list of possible chat categories is not final
   * (and will never be such). If your chat type belongs to a category which does not exist yet,
   * contact the person responsible for the code and ask him to add a new category. In the meanwhile
   * (or if you are happy with it), use <code>OTHER_CHAT_CATEGORY</code>
   */
  public ChatEvent(
      Connection conn,
      String clientTag,
    ...
    if (type == null) throw new IllegalArgumentException("ChatEvent type may not be null");
  ...
  /**
   * Returns the type of this message. This is a server specific string and should be dealt by a
   * server specific class.
   */
  ...
  /**
   * Returns the category of the chat. Note that the list of possible categories is not final (and
   * will never be), so don't make your code assume that the category is one of the current
   * categories.
   */
  ...
  /**
   * Returns the forum on which the message was sent. The forum identifies a certain instance of a
   * chat type. For room/channel tells this may be a <code>String/Integer</code> specifying the
   * room/channel name/number. For chat types with only a single instance (such as shouts,
   * announcements) or where the instance is already identified by the sender (personal tells) it
   * may be <code>null</code>. The actual type is server-specific, except that for events of
