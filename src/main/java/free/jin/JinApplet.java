...
public class JinApplet extends Applet implements JinContext {
  ...
  /**
   * Autologin parameters for Jin. They override any applet parameters in <code>getParameter</code>.
   */
  ...
  /**
   * Restarts JinApplet. This is called when the user closes Jin and is supposed to bring the applet
   * to its initial state - ready to accept a new username and password.
   */
  ...
  /**
   * Starts Jin with the specified application <code>Preferences</code> and list of known users.
   * Autologin parameters are set to login with the specified username and password.
   */
  private void start(
      Preferences prefs, User guestUser, User[] users, String username, String password) {
  ...
  private Server loadServer()
    ...
    if (portString != null) server.setPort(Integer.parseInt(portString));

    // Required to override possibly out of date user settings
    autologinParams.put("login.hostname", getDocumentBase().getHost());
    if (portString != null) autologinParams.put("login.ports", portString);
  ...
  /**
   * Overrides the <code>getParameter</code> method to allow us to add our own parameters, via the
  ...
  public Map getResources(String resourceType, Plugin plugin) {
    ...
    try {
      ...
      if (!IOUtilities.isURLCached(definitionsFileURL)) IOUtilities.cacheURL(definitionsFileURL);
      ZipInputStream zip =
          new ZipInputStream(IOUtilities.inputStreamForURL(definitionsFileURL, true));
      ZipEntry entry;
      while ((entry = zip.getNextEntry()) != null) {
        String entryName = entry.getName();
        if (!entryName.endsWith("definition") || entry.isDirectory()) continue;
  ...
  /**
   * Returns the resource with the specified type and id, or <code>null</code> if the resource can't
   * be loaded.
   */
  @Override
  ...
  private Resource loadResource(URL url, Plugin plugin) throws IOException {
    ...
    try {
      ...
      if (resource.load(url, plugin)) return resource;
      else return null;
  ...
  public void shutdown() {
    if (username != null) { // Not logged in as guest
      ...
      settingsUploadThread =
          new Thread() {
              ...
              try {
                ...
                while (!"OK".equals(result = uploadSettings())) {
                  ...
                  if (result.toLowerCase().indexOf("password") != -1) {
                    if (!showPasswordDialog(result)) break;
                  } else showErrorDialog(result);
                }
              } catch (IOException e) {
            ...
            private void showErrorDialog(String message) {
              Dialog errorDialog =
                  new SettingsUploadErrorDialog(
                      AWTUtilities.frameForComponent(JinApplet.this), message);
            ...
            private boolean showPasswordDialog(String message) {
              PasswordDialog passDialog =
                  new PasswordDialog(
                      AWTUtilities.frameForComponent(JinApplet.this), message, username);
      ...
      settingsUploadDialog =
          new SettingsUploadDialog(AWTUtilities.frameForComponent(this)) {
            @Override
  ...
  private String uploadSettings() throws IOException {
    ...
    for (int i = 0; i < users.length; i++) writeUser(out, users[i]);
  ...
  /**
   * Writes (for storage purposes) the information about the specified user into the specified
   * output stream.
   */
  ...
  /**
   * Sets the list of known user accounts, so that they can be uploaded as a part of the
   * preferences.
   */
  @Override
  ...
  public String getPasswordSaveWarning() {
    try {
      boolean isSecure =
          getPrefsDownloadUrl().getProtocol().equals("https")
              && getPrefsUploadUrl().getProtocol().equals("https");

      if (isSecure) return null;
      else return l10n.getString("passwordSaveWarning");
  ...
  private class UserAuthPanel extends Panel implements Runnable {
    ...
    private void createUI() {
      ...
      guestButton.addActionListener(
          new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
              setStatus(
                  l10n.getFormattedString("startingStatus", new Object[] {Jin.getAppName()}),
                  Color.black);
              loginButton.setEnabled(false);
              guestButton.setEnabled(false);
              startAsGuest();
    ...
    public void run() {
      try {
        ...
        if ("OK".equals(returnCode)) {
          ...
          for (int i = 0; i < users.length; i++) users[i] = loadUser(dataIn);
        ...
        setStatus(
            l10n.getFormattedString("stayOnPageStatus", new Object[] {Jin.getAppName()}),
            Color.black);
      } catch (IOException e) {
        e.printStackTrace();
        createErrorUI(e);
    ...
    private User loadUser(DataInputStream in) throws IOException {
      ...
      Preferences prefs =
          Preferences.load(new ByteArrayInputStream(IOUtilities.read(in, prefsLength)));
  ...
  private abstract class SettingsUploadDialog extends Dialog {
    ...
    public SettingsUploadDialog(Frame parent) {
      ...
      this.add(
          new Label(
              l10n.getFormattedString(
                  "prefsUploadDialog.message", new Object[] {Jin.getAppName()})));
  ...
  private class SettingsUploadErrorDialog extends Dialog {
    ...
    /**
     * Creates a new <code>SettingsUploadErrorDialog</code> with the specified parent frame and
     * error text.
     */
    public SettingsUploadErrorDialog(Frame parent, String errorMessage) {
      super(
          parent,
          l10n.getFormattedString("prefsUploadErrorDialog.title", new Object[] {Jin.getAppName()}),
          true);
    ...
    private void createUI(String errorMessage) {
      ...
      this.add(
          ...
          new Label(
              l10n.getFormattedString(
                  "prefsUploadErrorDialog.message", new Object[] {Jin.getAppName()})));
  ...
  private class PasswordDialog extends Dialog {
    ...
    /**
     * Creates a new <code>PasswordDialog</code> with the specified parent frame, error text and
     * current username.
     */
    public PasswordDialog(Frame parent, String errorMessage, String username) {
      super(
          parent,
          l10n.getFormattedString(
              "passwordPrefsUploadErrorDialog.title", new Object[] {Jin.getAppName()}),
          true);
    ...
    private void createUI(String errorMessage) {
      ...
      this.add(
          ...
          new Label(
              l10n.getFormattedString(
                  "passwordPrefsUploadErrorDialog.message", new Object[] {Jin.getAppName()})));
      ...
      Button retryButton =
          new Button(l10n.getString("passwordPrefsUploadErrorDialog.retryButton.text"));
      Button closeButton =
          new Button(l10n.getString("passwordPrefsUploadErrorDialog.closeButton.text"));
