...
public class PluginContext{
  ...
  public void setAndStart() throws PluginStartException{
    ...
    // Start the plugins
    for (int i = 0; i < plugins.length; i++)
      startPlugin(i, started);
  }
  
  
  
  /**
   * Starts the plugin with the specified index and marks it as started in the
   * specified flag array. If the plugin depends on any other plugins, they are
   * started first.
   */
  
  private void startPlugin(int pluginIndex, boolean [] started) throws PluginStartException{
    if (started[pluginIndex] || !isPluginActive[pluginIndex])
      return;
    ...
    for (int i = 0; i < dependencies.length; i++){
      ...
      if (dependency >= 0)
        startPlugin(dependency, started);
    }
  ...
  /**
   * Returns the index of the plugin with the specified id. Returns
   * <code>-1</code> if none.
   */
  ...
  public Plugin getPlugin(String id){
    ...
    if ((index >= 0) && isPluginActive[index])
