...
public class User {
  ...
  /**
   * Creates a new <code>User</code> object. Before creating a <code>User</code> object with a user
   * supplied username, make sure that the username is not the guest username (via the username
   * policy of the server).
   *
   * @param server The server of the account.
   * @param username The username of the account.
   * @param prefs The user's preferences.
   * @param files The user's files. Maps filenames to <code>MemoryFile</code> objects.
   */
  ...
  /**
   * Creates a new <code>User</code> object with the specified server and username. The user
   * initially has no preferences or files.
   */
  ...
  /**
   * Returns the preferences of this account, backed up by the application's customizing
   * preferences.
   */
  public Preferences getPrefs() {
    Preferences customizingPrefs = Jin.getInstance().getCustomizingPrefs();
    if (customizingPrefs == null) return prefs;
  ...
  /**
   * Returns the file with the specified name, or <code>null</code> if no file with the specified
   * name exists. Filenames are case sensitive.
   */
  ...
  /**
   * Sets the file with the specified name. If a file with the specified name already exists, it is
   * deleted. The specified file may be <code>null</code> if you want to call this method just for
   * the side effect of removing the existing file.
   */
  public void setFile(String filename, MemoryFile file) {
    if (file == null) files.remove(filename);
  ...
  public ConnectionDetails getPreferredConnDetails() {
    ...
    if (isGuest()) return ConnectionDetails.createGuest(getServer(), username, hostname, ports);
    else
      return ConnectionDetails.create(
  ...
  void setPreferredConnDetails(ConnectionDetails details) {
    ...
    if (details.getUser() != this) throw new IllegalArgumentException("user property mismatch");
