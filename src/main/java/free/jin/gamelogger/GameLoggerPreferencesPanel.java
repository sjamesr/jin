...
public class GameLoggerPreferencesPanel extends PreferencesPanel {
  ...
  /**
   * This variable is set to <code>true</code> when the values of the rule fields is being changed
   * programmatically because a new rule is selected. This is done so that the DocumentListener
   * knows not to fire a ChangeEvent.
   */
  private boolean ignoreRuleFieldsDocumentChange = false;

  /**
   * This variable is set to <code>true</code> when the selection in the rules list is being changed
   * programmatically. This is done so that the corresponding selection listener can act
   * accordingly.
   */
  private boolean ignoreRulesListSelectionChange = false;

  /**
   * The currently selected index in the rules list. We need to keep this in order to be able to
   * roll back if the user tries to change the selection when the current selection is incomplete.
   */
  private int rulesListSelectedIndex = -1;

  /**
   * A DocumentListener we register with all text components that require a change event to be fired
   * by this PreferencesPanel whenever their text changes.
   */
  private final DocumentListener changeFiringDocumentListener =
      new DocumentListener() {
        @Override
        public void changedUpdate(DocumentEvent e) {
          if (!ignoreRuleFieldsDocumentChange) fireStateChanged();
        }
  ...
  protected void syncUI() {
    try {
      ...
      if (loggingRulesList.getSelectedIndex() == -1) {
        ...
        if (selectedItem
            instanceof LoggingRule) { // When a new rule is created, this value is merely a String
  ...
  protected void createUI() {
    ...
    ActionListener loggingModeListener =
        new ActionListener() {
          @Override
    ...
    switch (loggingMode) {
      case GameLogger.LOG_NONE:
        logNoneButton.setSelected(true);
        break;
      case GameLogger.LOG_ALL:
        logAllButton.setSelected(true);
        break;
      case GameLogger.USE_RULES:
        useRulesButton.setSelected(true);
        break;
    }
    ...
    browseAllGamesLogFileButton.addActionListener(
        new PGNFileSelectActionListener(this, allGamesLogFileField));
    ...
    loggingRulesList.addListSelectionListener(
        new ListSelectionListener() {
          ...
          public void valueChanged(ListSelectionEvent evt) {
            ...
            if ((rulesListSelectedIndex != -1)
                && (rulesListSelectedIndex < rulesListModel.size())) {
              try {
    ...
    JScrollPane loggingListRulesScrollPane = new JScrollPane(loggingRulesList);
    loggingListRulesScrollPane.setVerticalScrollBarPolicy(
        ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS);
    loggingListRulesScrollPane.setHorizontalScrollBarPolicy(
    ...
    addRuleButton.addActionListener(
        new ActionListener() {
          ...
          public void actionPerformed(ActionEvent evt) {
            ...
            rulesListModel.addElement(
                I18n.get(GameLoggerPreferencesPanel.class)
                    .getString("initialNewGameLoggingRuleName"));
            ...
            new DocumentListener() {
              ...
              private void updateRulename() {
                ...
                if (selectedItem instanceof String) {
                  ...
                  if (text.length() == 0)
                    rulesListModel.setElementAt(
                        I18n.get(GameLoggerPreferencesPanel.class)
                            .getString("initialNewGameLoggingRuleName"),
                        loggingRulesList.getSelectedIndex());
                  else rulesListModel.setElementAt(text, loggingRulesList.getSelectedIndex());
                } else if (selectedItem instanceof LoggingRule) {
                  LoggingRule loggingRule = (LoggingRule) selectedItem;
                  String text = rulenameField.getText();
                  if ((text != null)
                      && (text.length() != 0)) { // A temporary state when setting text?
    ...
    helpConditionButton.addActionListener(
        new ActionListener() {
          @Override
          public void actionPerformed(ActionEvent evt) {
            I18n i18n = I18n.get(GameLoggerPreferencesPanel.class);
    ...
    int maxPrefWidth =
        Math.max(
            rulenameLabel.getPreferredSize().width,
            Math.max(
                filenameLabel.getPreferredSize().width, conditionLabel.getPreferredSize().width));
  ...
  /**
   * Enables or disables all the components within the specified container. This is a rather hacky
   * method, which is very specific to this class, as it's impossible to implement the desired
   * functionality in a generic manner.
   */
  private static void setContainerEnabled(Container container, boolean enabled) {
    ...
    for (int i = 0; i < children.length; i++) {
      ...
      if (child instanceof JTextComponent) ((JTextComponent) child).setEditable(enabled);
      child.setEnabled(enabled);

      if (child instanceof Container) setContainerEnabled((Container) child, enabled);
    }
  }
  ...
  private void updateRuleFromUI(int ruleIndex) throws BadChangesException {
    ...
    if ((rulename == null) || (rulename.length() == 0)) {
      ...
      if (errorMessage != null) errorComponent = filenameField;
    }
    ...
    try {
      ...
      throw new BadChangesException(
          i18n.getString("invalidGameLoggingConditionMessage"), conditionField);
    }
  }

  /**
   * Interprets the filename selected by the user by returning the filename to be the actual logging
   * target.
   */
  private static String interpretLoggingTarget(String filename) {
    if (new File(filename).isAbsolute()) return filename;
    else return new File(System.getProperty("user.home"), filename).getAbsolutePath();
  }
  ...
  private String checkLogFile(String filename) {
    ...
    Object[] messageArgs =
        new Object[] {logFile.getPath(), logDir == null ? null : logDir.getPath()};
    if (logFile.isDirectory())
      return i18n.getFormattedString("logFileIsDirectoryErrorMessage", messageArgs);
    else if (logDir == null) return i18n.getFormattedString("badLogFileErrorMessage", messageArgs);
  ...
  public void applyChanges() throws BadChangesException {
    ...
    int selectedIndex = loggingRulesList.getSelectedIndex();
    if (selectedIndex != -1) updateRuleFromUI(selectedIndex);

    String loggingModeString;
    if (logNoneButton.isSelected()) loggingModeString = "none";
    else if (logAllButton.isSelected()) loggingModeString = "all";
    else if (useRulesButton.isSelected()) loggingModeString = "rules";
    else throw new IllegalStateException("None of the mode radio buttons are selected");
    ...
    if ("all".equals(loggingModeString)) {
      ...
      else errMessage = checkLogFile(allGamesLogFile);

      if (errMessage != null) throw new BadChangesException(errMessage, allGamesLogFileField);
    }
    ...
    for (int i = 0; i < rulesCount; i++) {
      ...
      prefs.setString(
          "logging.rule-" + (i + 1) + ".filename", interpretLoggingTarget(rule.getFilename()));
    }
  ...
  /**
   * A small ActionListener implementation which pops up a FileChooserDialog when activated and lets
   * the user choose a pgn file. It then sets the selected filename to the specified JTextField.
   */
  private static class PGNFileSelectActionListener implements ActionListener {
    ...
    /**
     * Creates a new PGNFileSelectActionListener with the specified parent component and textfield.
     */
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      if (result == JFileChooser.APPROVE_OPTION) {
        ...
        if (path == null) path = fileChooser.getSelectedFile().getAbsolutePath();

        if (!(path.endsWith(".pgn") || path.endsWith(".PGN"))) path = path + ".pgn";
