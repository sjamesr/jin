 ...
 * on freechess.org for information about the format.
 */
public class GameInfoStruct extends Struct {
  ...
  public GameInfoStruct(
      int gameNumber,
      boolean isGamePrivate,
      String gameCategory,
      boolean isGameRated,
      boolean isWhiteRegistered,
      boolean isBlackRegistered,
      int whiteTime,
      int whiteInc,
      int blackTime,
      int blackInc,
      int partnerGameNumber,
      int whiteRating,
      char whiteProvShow,
      int blackRating,
      char blackProvShow,
      boolean isWhiteTimesealed,
      boolean isBlackTimesealed) {

    super(20);

    if (whiteTime < 0)
      throw new IllegalArgumentException(
          "White's initial time (" + whiteTime + ") may not be negative");

    if (blackTime < 0)
      throw new IllegalArgumentException(
          "Black's initial time (" + blackTime + ") may not be negative");

    if (whiteInc < 0)
      throw new IllegalArgumentException(
          "White's increment (" + whiteInc + ") may not be negative");

    if (blackInc < 0)
      throw new IllegalArgumentException(
          "Black's increment (" + blackInc + ") may not be negative");
  ...
  /**
   * Parses a gameinfo line and returns a corresponding GameInfoStruct object. Sample line:<br>
  ...
  /**
   * Returns the game category. This will be the rating category in case of chess games and the wild
   * variant name in case of a wild game.
   */
  ...
  /**
   * Returns white's rating provshow character. 'E' if the rating is estimated, 'P' if provisional,
   * and ' ' if neither.
   */
  ...
  /**
   * Returns black's rating provshow character. 'E' if the rating is estimated, 'P' if provisional,
   * and ' ' if neither.
   */
