 ...
 * HREF="http://www.freechess.org/WWWhelp/style12.html"
 * >http://www.freechess.org/WWWhelp/style12.html</A> or the "help style12" helpfile on
 * freechess.org for information about the format.
 */
public class Style12Struct extends Struct {
  ...
  /**
   * Creates a new Style12Struct with the specified arguments. You would usually want to use the
   * <code>parseStyle12Line</code> method and not this constructor to obtain Style12Struct objects.
   *
   ...
   */
  public Style12Struct(
      String boardLexigraphic,
      String currentPlayer,
      int doublePawnPushFile,
      ...
      int initTime,
      int increment,
      int whiteMaterialStrength,
      int blackMaterialStrength,
      int whiteTime,
      int blackTime,
      int nextMoveNumber,
      String moveVerbose,
    ...
    if ((doublePawnPushFile < -1) || (doublePawnPushFile > 7))
      throw new IllegalArgumentException(
          "Bad value for double pawn push file: " + doublePawnPushFile);

    if (boardLexigraphic == null) throw new IllegalArgumentException("Board may not be null");

    if (boardLexigraphic.length() != 64)
      throw new IllegalArgumentException(
          "Board string length (" + boardLexigraphic.length() + ") must be 64 characters");

    if ((currentPlayer == null) || ((!currentPlayer.equals("W")) && (!currentPlayer.equals("B"))))
      throw new IllegalArgumentException(
          "Current player string (" + currentPlayer + ") must be either \"W\" or \"B\"");

    if (pliesSinceIrreversible < 0)
      throw new IllegalArgumentException(
          "Plies since irreversible move (" + pliesSinceIrreversible + ") cannot be negative");

    if (whiteName == null) throw new IllegalArgumentException("White name may not be null");

    if (blackName == null) throw new IllegalArgumentException("Black name may not be null");
    ...
    if (whiteMaterialStrength < 0)
      throw new IllegalArgumentException(
          "White's material strength (" + whiteMaterialStrength + ") may not be negative");

    if (blackMaterialStrength < 0)
      throw new IllegalArgumentException(
          "Black's material strength (" + blackMaterialStrength + ") may not be negative");

    if (nextMoveNumber <= 0)
      throw new IllegalArgumentException(
          "The next move number (" + nextMoveNumber + ") must be positive");

    if (moveTime < 0)
      throw new IllegalArgumentException(
          "The amount of time taken by the last move (" + moveTime + ") may not be negative");
    ...
    if (lag < 0)
      throw new IllegalArgumentException(
          "Lag may not be negative (really, it's against the laws of physics)");
  ...
  public static Style12Struct parseStyle12Line(String line) {
    ...
    for (int i = 0; i < 8; i++) posBuf.append(tokens.nextToken()); // The board
    ...
    boolean canWhiteCastleQueenside =
        parseBoolean(tokens.nextToken()); // Can white castle queenside
    boolean canBlackCastleKingside = parseBoolean(tokens.nextToken()); // Can black castle kingside
    boolean canBlackCastleQueenside =
        parseBoolean(tokens.nextToken()); // Can black castle queenside

    int pliesSinceIrreversible =
        Integer.parseInt(tokens.nextToken()); // Number of plies since an irreversible move
    ...
    if (moveVerbose.equals("none")) moveVerbose = null;
    ...
    return new Style12Struct(
        ...
        initTime,
        increment,
        whiteMaterialStrength,
        blackMaterialStrength,
        whiteTime,
        blackTime,
        nextMoveNumber,
        moveVerbose,
  ...
  /**
   * If the given string is "1", return <code>true</code>. If it's "0", returns <code>false</code>.
  ...
  public String getBoardFEN() {
    ...
    if (canWhiteCastleKingside()) castlingAvailability.append('K');
    if (canWhiteCastleQueenside()) castlingAvailability.append('Q');
    if (canBlackCastleKingside()) castlingAvailability.append('k');
    if (canBlackCastleQueenside()) castlingAvailability.append('q');
    ...
    if (getDoublePawnPushFile() == -1) enPassantSquare = "-";
    else {
  ...
  /**
   * Returns the file of the double pawn push on the last move, or -1 if the last move wasn't a
   * double pawn push.
   */
  ...
  /**
   * Returns <code>true</code> if white can still castle kingside. Returns <code>false</code>
   * otherwise.
   */
  ...
  /**
   * Returns <code>true</code> if white can still castle queenside. Returns <code>false</code>
   * otherwise.
   */
  ...
  /**
   * Returns <code>true</code> if black can still castle kingside. Returns <code>false</code>
   * otherwise.
   */
  ...
  /**
   * Returns <code>true</code> if black can still castle queenside. Returns <code>false</code>
   * otherwise.
   */
  ...
  /**
   * Returns the amount of half moves made since the last irreversible move, or 0 if the last move
   * was irreversible.
   */
  ...
  /**
   * Returns the game type. Possible values are <code>MY_GAME</code>, <code>OBSERVED_GAME</code> and
  ...
  /**
   * Returns <code>true</code> if it's the user's turn, <code>false</code> if it's the opponent's
  ...
  public boolean isMyTurn() {
    if ((getGameType() != MY_GAME) || !isPlayedGame())
      throw new IllegalStateException(
          "Either the game type is not MY_GAME or it's not a played game.");
  ...
  /**
   * Returns the number of the next move. (standard chess numbering -- White's and Black's first
   * moves are both 1, etc.)
   */
  ...
  /**
   * Returns the amount of plies since the beginning of the game. The method assumes that the first
   * player to move was white.
   */
  ...
  /**
   * Returns a verbose representation of the move, or <code>null</code> if none. The format is
   * either "o-o", "o-o-o" or something like "Q/c7-c5" or for promotions "P/g7-h8=Q".
   */
  ...
  /**
   * Returns a SAN (Standard Algebraic Notation) representation of the move, or <code>null</code> if
   * none.
   */
  ...
  /**
   * Returns the amount of lag incurred when making the move that caused this style12 message to be
   * sent.
   */
  public int getLag() {
