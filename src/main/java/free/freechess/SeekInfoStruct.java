 ...
 * available in the "help iv_seekinfo" file on the Free Internet Chess Server
...
public class SeekInfoStruct extends Struct {
  ...
  public SeekInfoStruct(
      ...
      int minRating,
      int maxRating,
      boolean isAutomaticAccept,
      boolean isFormulaUsed) {

    if (time < 0)
      throw new IllegalArgumentException(
          "The game's initial time (" + time + ") may not be negative");
  ...
  /**
   * Parses the given seekinfo line and returns a corresponding SeekInfoStruct object. Here's an
   * example seekinfo line:
   *
   * <PRE>
   * <s> 29 w=Snaps ti=02 rt=1532  t=1 i=0 r=r tp=lightning c=? rr=0-9999 a=t f=t
   * </PRE>
   *
   * If the seek cannot be accepted by the user, the line identifier is "<sn>" and not "<s>". More
   * information about the format is available in the FICS iv_seekinfo help file.
   */
  ...
  /**
   * Checks that the next token for the given StringTokenizer is the specified String. Throws an
  ...
  /**
   * Returns <code>true</code> if the user account can accept the seek, returns <code>false</code>
   * otherwise.
   */
  ...
  /**
   * Returns the titles of the seeking player, ORed into an int. To find whether the player is GM,
   * for example, use <code>(getSeekerTitles() & SeekInfoStruct.GM) != 0</code>.
   */
  ...
  /**
   * Returns the seeker rating's provshow character. 'E' if the rating is estimated, 'P' if
   * provisional, and ' ' if neither.
   */
  ...
  /**
   * Returns the sought game type. Either the name of the wild variant, or the rating type, if the
   * variant is chess. For example - "suicide", "lightning", "blitz" etc.
   */
  ...
  /**
   * Returns a character specifying the desired color for the seeker. 'W' if white, 'B' if black,
   * and '?' if the seeker doesn't care.
   */
  ...
  /**
   * Returns <code>true</code> if the match will be accepted automatically, <code>false</code> if
   * the seeker must confirm it manually.
   */
  ...
  /**
   * Returns <code>true</code> if in order to accept the seek, one must pass the formula of the
   * seeker. Returns <code>false</code> otherwise.
   */
