...
public class ColorChooser extends JComponent implements Mnemonicable {
  ...
  /**
   * Creates a new <code>ColorChooser</code> with the specified text and the initial color of black.
   */
  ...
  public ColorChooser(String text, Color initialColor) {
    ...
    if (SwingUtils.isMacLnF()) button.setMargin(new Insets(5, 5, 5, 5));
    else button.setMargin(new Insets(3, 3, 3, 3));
          ...
          public void actionPerformed(ActionEvent evt) {
            Color newColor =
                JColorChooser.showDialog(
                    SwingUtilities.windowForComponent(ColorChooser.this),
                    LocalizationService.getForClass(ColorChooser.class).getString("dialogTitle"),
                    color); //$NON-NLS-1$
