...
/**
 * A PlainDocument extension which allows to disallow certain changes, by implementing the
 * isChangeAccepted(String, String) method and returning false. A beep is emitted when an attempt to
 * make an unacceptable change is made.
 */
public abstract class StrictPlainDocument extends PlainDocument {
  ...
  /**
   * Overrides PlainDocument.insertString(int, String, AttributeSet) to check whether the change is
   * accepted before applying the change.
   */
  @Override
  public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
    ...
    if (isChangeAccepted(oldString, newString)) super.insertString(offs, str, a);
    else Toolkit.getDefaultToolkit().beep();
  }

  /**
   * Returns true if the change from the old text to the new text is acceptable, returns false
   * otherwise.
   */
