...
public class FullscreenPanel extends FixedJPanel {
  ...
  /**
   * Are we in real fullscreen mode (as opposed to fake fullscreen mode)? Only relevant if we really
   * are in fullscreen mode
   */
  private boolean isRealFullscreen;

  /**
   * The extended state of the original parent frame. This is needed as a workaround for the
   * maximized state sometimes being reset when coming back from fullscreen mode.
   */
  private int originalFrameState;

  /**
   * The original parent frame. We need to keep this because the fullscreen panel might get removed
   * from its parent, but we still need to make the original parent frame visible when the target
   * panel is made "normal" again.
   */
  ...
  public FullscreenPanel(Component target) {
    ...
    fullscreenModeModel.addListener(
        new BooleanListener() {
          @Override
  ...
  public void remove(int index) {
    if (beingModified) super.remove(index);
      ...
      throw new IllegalStateException(
          "You may not remove components from FullscreenPanel"); //$NON-NLS-1$
  }

  /**
   * Sets whether we are allowed to use fullscreen exclusive mode (if available). If not, fullscreen
   * mode will always be faked via an undecorated frame placed over the entire screen.
   */
  ...
  private void makeFullscreen() {
    ...
    } else fullscreenFrame = setFakeFullscreen();
  ...
  /**
   * Attempts to set the specified window in real fullscreen mode via the JDK 1.4 fullscreen API.
   */
  private JFrame setRealFullscreen() {
    ...
    GraphicsDevice graphicsDevice =
        SwingUtils.frameForComponent(this).getGraphicsConfiguration().getDevice();
  ...
  /**
   * Fakes a fullscreen mode, merely setting the specified window's size to the size of the screen.
   */
  ...
  private void makeNormal() {
    ...
    if (isRealFullscreen) makeRealNormal();
    else makeFakeNormal();
