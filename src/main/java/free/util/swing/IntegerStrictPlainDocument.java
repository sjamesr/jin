...
/** A StrictPlainModel which only allows inputting an integer value in a certain range. */
public class IntegerStrictPlainDocument extends StrictPlainDocument {
  ...
  /**
   * Creates a new IntegerStrictPlainDocument which will only allow inputting integer values in the
   * given radix in the range [minValue..maxValue].
   *
  ...
  /**
   * Returns true if the new text represents an integer in the correct range when parsed using the
   * appropriate radix, returns false otherwise.
   */
  @Override
  public boolean isChangeAccepted(String oldText, String newText) {
    ...
    try {
      ...
      if ((val < getMinValue()) || (val > getMaxValue())) return false;
      else return true;
