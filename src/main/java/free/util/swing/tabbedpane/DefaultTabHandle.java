...
public class DefaultTabHandle implements TabHandle {
  ...
  /**
   * Creates a new <code>DefaultTabHandle</code> for the specified tab to be used in the specified
   * tabbed pane.
   */
  public DefaultTabHandle(TabbedPane tabbedPane, Tab tab) {
    ...
    tab.addPropertyChangeListener(
        new PropertyChangeListener() {
          @Override
          public void propertyChange(PropertyChangeEvent evt) {
            String propertyName = evt.getPropertyName();

            if ("title".equals(propertyName)) confLabelFromTab();
            else if ("icon".equals(propertyName)) confLabelFromTab();
            else if ("closeable".equals(propertyName)) confComponentFromTab();
          }
        });
  }
  ...
  private void confComponentFromTab() {
    ...
    if (tab.isCloseable()) {
      if (SwingUtils.isMacLnF()) component.add(closeButton, BorderLayout.LINE_START);
      else component.add(closeButton, BorderLayout.LINE_END);
    }
  ...
  /**
   * Invoked when the user presses the close button. Consults the <code>TabCloseApprover</code> of
   * the tabbed pane and closes the tab.
   */
