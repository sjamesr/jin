...
public class TabbedPaneEvent extends EventObject{
  ...
  public TabbedPaneEvent(TabbedPaneModel source, int id, int tabIndex){
    ...
    if ((tabIndex < 0) || (tabIndex > source.getTabCount()) || 
        ((tabIndex == source.getTabCount()) && (id != TAB_REMOVED)))
      throw new IndexOutOfBoundsException("tabIndex (" + tabIndex + ") out of bounds (tabCount="+ source.getTabCount() + ")");
  ...
  /**
   * Returns the index of the tab affected in this event. In the case of a
   * <code>TAB_REMOVED</code> event, this is the index of the tab before it was
   * removed.
   */
