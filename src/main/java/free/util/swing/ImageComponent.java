...
public class ImageComponent extends JComponent {
  ...
  /**
   * Creates a new <code>ImageComponent</code> with the specified image (may be <code>null</code>).
   */
    ...
    addMouseListener(
        new MouseAdapter() {
          ...
          public void mouseEntered(MouseEvent e) {
            if (!isPopupImage() || !isImageSizeKnown()) return;
            ...
            if (popup == null) {
              ...
              ic.addMouseMotionListener(
                  new MouseMotionAdapter() {
                    @Override
                    public void mouseMoved(MouseEvent evt) {
                      if (!evt.getPoint().equals(mouseLocationInPopup)) popup.setVisible(false);
                    }
                  });
              ...
              popup.setPreferredSize(
                  AWTUtilities.augmentSize(ic.getPreferredSize(), tip.getInsets()));
            }
          ...
          public void mouseExited(MouseEvent e) {
            if (popupTimer.isRunning()) popupTimer.stop();
            if (popup != null) {
              ...
              SwingUtilities.invokeLater(
                  new Runnable() {
                    @Override
                    public void run() {
                      if (!popupUnderCursor) popup.setVisible(false);
                    }
                  });
            }
          }
  ...
  public void setImage(Image image) {
    ...
    if (image != null) getToolkit().prepareImage(image, -1, -1, this);
  ...
  public void paintComponent(Graphics g) {
    ...
    if ((imageWidth == -1) || (imageHeight == -1)) g.drawImage(image, 0, 0, width, height, this);
    else {
      double ratio = ((double) width) / height;
      double imageRatio = ((double) imageWidth) / imageHeight;

      if (ratio > imageRatio) g.drawImage(image, 0, 0, (int) (height * imageRatio), height, this);
      else g.drawImage(image, 0, 0, width, (int) (width / imageRatio), this);
    }
  }
