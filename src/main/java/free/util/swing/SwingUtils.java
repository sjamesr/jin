...
public class SwingUtils {
  ...
  /**
   * Adds a keyboard action to the specified <code>RootPaneContainer</code> so that it is disposed
   * when the ESCAPE key is hit. <B>Note that this only works for subclasses of <code>Window</code>
   * .</B>
   */
   ...
   * exists. This does the same as <code>free.util.AWTUtilities.frameForComponent</code> but also
   * takes care of various swing quirks.
   */
  ...
  /**
   * Configures the specified button to be an icon button. The button is made unfocusable, non
   * default capable, non painting its border or content area and marginless. Additionally, unless
   * the specified image is <code>null</code>, variants of the image to be used as rollover, pressed
  ...
  /**
   * Returns the displayed mnemonic index for the specified label specification. A label
   * specification is a string which possibly includes an ampersand; the character following the
   * first ampersand is the mnemonic.
   */
  public static int getLabelMnemonicIndex(String labelSpec) {
    if (labelSpec == null) return -1;

    return labelSpec.indexOf('&');
  }

  /**
   * Returns the text for the specified label specification. A label specification is a string which
   * possibly includes an ampersand; the character following the first ampersand is the mnemonic.
   */
  public static String getLabelText(String labelSpec) {
    if (labelSpec == null) return null;

    int ampIndex = labelSpec.indexOf('&');
    if (ampIndex == -1) return labelSpec;
  ...
  public static JLabel[] labelsForLines(String text) {
    ...
    for (int i = 0; i < labels.length; i++) labels[i] = new JLabel(tokenizer.nextToken());
  ...
  /**
   * Links the enabled state of the specified component to the selected state of the specified
   * <code>ListSelectionModel</code>.
   */
  public static void linkEnabledToSelected(
      final JComponent component, final ListSelectionModel model) {
    component.setEnabled(!model.isSelectionEmpty());
    model.addListSelectionListener(
        new ListSelectionListener() {
          @Override
          public void valueChanged(ListSelectionEvent e) {
            if (!e.getValueIsAdjusting()) component.setEnabled(!model.isSelectionEmpty());
          }
        });
  }

  /**
   * Returns a list model of the specified collection. Note that the list model copies the
   * references to the elements in the collection, so any changes in the collection are not
   * reflected in the model and vice versa.
   */
  public static ListModel collectionListModel(Collection collection) {
    DefaultListModel model = new DefaultListModel();
    for (Iterator i = collection.iterator(); i.hasNext(); ) model.addElement(i.next());
    return model;
  }
}
