...
public class TableLayout implements LayoutManager2 {
  ...
  /**
   * Creates a new TableLayout with the specified amount of columns, horizontal/vertical gaps
   * between columns/cells.
   */
  ...
  public void removeLayoutComponent(Component component) {
    synchronized (component.getTreeLock()) {
      ...
      for (int i = rowCount - 1; i >= 0; i--) {
        ...
        if (isEmpty) rows.removeElementAt(i);
        else break;
      }
    }
  }
  ...
  /**
   * Returns a matrix of Dimension objects specifying the preferred sizes of the components we are
   * going to layout.
   */
  ...
  /**
   * Calculates and returns a Pair where the first object is an array holding the column widths of
   * our layout and the second is the rowHeights.
   */
  ...
  /**
   * Lays out the specified container. Throws an <code>IllegalStateException</code> if any of the
   * components added via the <code>addLayoutComponent</code> method have a different parent than
  ...
  public void layoutContainer(Container parent) {
    synchronized (parent.getTreeLock()) {
      ...
      Dimension layoutSize =
          new Dimension(
              parentSize.width - xGap * (columnCount - 1) - parentInsets.left - parentInsets.right,
              parentSize.height - yGap * (rowCount - 1) - parentInsets.top - parentInsets.bottom);
      Dimension prefLayoutSize =
          new Dimension(
              prefParentSize.width
                  - xGap * (columnCount - 1)
                  - parentInsets.left
                  - parentInsets.right,
              prefParentSize.height
                  - yGap * (rowCount - 1)
                  - parentInsets.top
                  - parentInsets.bottom);
      ...
      int y =
          parentInsets.top
              + Math.max(
                  0, (int) (parent.getAlignmentY() * (layoutSize.height - prefLayoutSize.height)));

      for (int i = 0; i < rowCount; i++) {
        int x =
            parentInsets.left
                + Math.max(
                    0, (int) (parent.getAlignmentX() * (layoutSize.width - prefLayoutSize.width)));

        int cellHeight =
            (int)
                (rowHeights[i] * Math.min(1, ((double) layoutSize.height) / prefLayoutSize.height));
  ...
  /**
   * Calculates the preferred layout size for the specified preferred column widths and row heights.
   */
  private Dimension calculatePreferredLayoutSize(
      Container parent, int[] columnWidths, int[] rowHeights) {
    int prefWidth = 0;
    int prefHeight = 0;

    for (int i = 0; i < columnWidths.length; i++) prefWidth += columnWidths[i];
    for (int i = 0; i < rowHeights.length; i++) prefHeight += rowHeights[i];
