...
/**
 * A LayoutManager which lays out a single component making sure its height is always the same as
 * its width. If the parent container's width differs from it height, the child will be laid out
 * according to its x and y alignment.
 */
public class SquareLayout implements LayoutManager {
  ...
  /**
   * Returns the sole child of the specified container, or <code>null</code> if none. Throws an
   * <code>IllegalStateException</code> if there is more than one child.
   */
  private Component getChild(Container c) {
    int childCount = c.getComponentCount();
    if (childCount > 1) throw new IllegalStateException("May not layout more than one component");
    else if (childCount == 0) return null;
  ...
  public void layoutContainer(Container container) {
    ...
    child.setBounds(
        rect.x + (int) (widthSpace * child.getAlignmentX()),
        rect.y + (int) (heightSpace * child.getAlignmentY()),
        minSize,
        minSize);
  }
