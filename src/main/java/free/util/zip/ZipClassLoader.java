 ...
 * in order for the <code>getResource(String)</code> method of this classloader to work correctly,
 * the application must have a <code>URLStreamHandlerFactory</code> which returns an instance of
 * <code>free.util.zip.ZipURLStreamHandler</code> for the "zip" protocol.
 * <strong>IMPORTANT:</strong> This classloader doesn't seem to work well with compressed zipfiles
 * under MS VM (thinks the file is corrupted sometimes).
 */
public class ZipClassLoader extends ChildClassLoader {
  ...
  /**
   * Creates a new <code>ZipClassLoader</code> for the specified zip file with the specified parent
   * <code>ClassLoader</code> to which loading is delegated if a class or a resource can't be found.
   */
