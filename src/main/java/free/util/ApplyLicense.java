...
/**
 * A commandline utility application which adds text taken from a given file, and adds it to the
 * beginning of all files with the given extension in a given directory. This is useful for adding
 * license text to source code files. The modifying process of a single file includes renaming it to
 * a file with the name of the original file, ".tmp" added to it, then creating a new file with the
 * name of the original file, writing license information into the new file, writing the contents of
 * the ".tmp" file into the new file and only then deleting the ".tmp" file. If at any point an I/O
 * error occurrs, the utility terminates immediately. This ensures that at all times, either the
 * original file or the ".tmp" file exist, preventing possible data loss.
 */
public class ApplyLicense {
  ...
  public static void main(String[] args) {
    ...
    while (argIndex < args.length - 3) {
      ...
      if (arg.equals("-r")) recurse = true;
      else if (arg.equals("-b")) backupDirName = args[++argIndex];
      else if (arg.equals("-u")) undo = true;
    ...
    try {
      ...
      if (undo) removeLicense(license, new ExtensionFilenameFilter("." + extension), dir, recurse);
      else applyLicense(license, new ExtensionFilenameFilter("." + extension), dir, recurse);
    } catch (IOException e) {
      e.printStackTrace();
      System.exit(0);
    }
  ...
  /**
   * Prints usage information to the standard output stream. Here is what this method prints:
   *
   * <PRE>
   *
  ...
  /**
   * Adds the given license string to the beginning of every file in the given directory, optionally
   * recursing into subdirectories, which passes the given FilenameFilter.
   *
   * @throws IOException If an I/O error occurs during the processing of the file(s).
   */
  public static void applyLicense(String license, FilenameFilter filter, File dir, boolean recurse)
      throws IOException {
    if (!dir.exists())
    ...
    for (int i = 0; i < filenames.length; i++) {
      ...
      if (file.isDirectory()) {
        if (recurse) applyLicense(license, filter, file, true);
  ...
  private static void prependLicense(String license, File file) throws IOException {
    ...
    if (!tmpFile.delete()) throw new IOException("Unable to delete " + tmpFile);
  }

  /**
   * Removes the given license string from the beginning of every file in the given directory,
   * optionally recursing into subdirectories, which passes the given FilenameFilter.
   *
   * @throws IOException If an I/O error occurs during the processing of the file(s).
   */
  public static void removeLicense(String license, FilenameFilter filter, File dir, boolean recurse)
      throws IOException {
    if (!dir.exists())
      throw new IllegalArgumentException("The directory " + dir + " does not exist");
    ...
    for (int i = 0; i < filenames.length; i++) {
      ...
      File file = new File(dir, filename);
      if (file.isDirectory()) {
        if (recurse) removeLicense(license, filter, file, true);
      } else if (filter.accept(dir, filename)) {
  ...
  /**
   * Prepends the specified text from the beginning of the given file. Returns <code>true</code> if
   * the file indeed started with the specified text, false otherwise.
   */
