...
public class ImageUtilities {
  ...
  /**
   * Starts loading the given image, returns only when it's done loading. Note that it's much more
  ...
  public static int preload(Image image) {
    ...
    if ((toolkit.checkImage(image, -1, -1, null) & ImageObserver.ALLBITS) != 0) return COMPLETE;
    ...
    synchronized (lock) {
      while (true) {
        ...
        if ((result & ImageObserver.ALLBITS) != 0) return COMPLETE;
        if ((result & ImageObserver.ERROR) != 0) return ERRORED;
        if ((result & ImageObserver.ABORT) != 0) return ABORTED;
  ...
  public static int[] preload(Image[] images, int[] results) {
    ...
    if ((results == null) || (results.length < images.length)) results = new int[images.length];
    Toolkit toolkit = Toolkit.getDefaultToolkit();
  ...
  /**
   * This class is an implementation of ImageObserver which notifies a given lock when loading of
   * the Image is done. This can be used to wait until a certain Image has finished loading.
   */
  public static class ImageLoadObserver implements ImageObserver {
    ...
    /**
     * Creates a new ImageLoadObserver which will notify the given lock when the Image is done
     * loading.
     */
    ...
    /**
     * If infoflags has the ALLBITS flag set, notifies the lock and returns false, otherwise simply
     * returns true.
     */
    @Override
     ...
     * Possible values are {@link ImageUtilities#COMPLETE}, {@link ImageUtilities#ERRORED} and
