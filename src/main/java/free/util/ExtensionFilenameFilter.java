...
/**
 * An implementation of the <code>java.io.FilenameFilter</code> which accepts files whose name ends
 * with a certain string. This makes it useful for accepting files with a certain extension.
 */
public class ExtensionFilenameFilter implements FilenameFilter {
  ...
  /**
   * Creates a new ExtensionFilenameFilter with the given string. Only files ending with that string
  ...
  /**
   * Creates a new ExtensionFilenameFilter with the given string array. Only files ending with one
   * of the strings in the given string array will be accepted by the created
   * ExtensionFilenameFilter. Note that in order to use this class to accept only files with a
   * certain extension, you must also provide the '.' character before the extension. For example,
   * to accept only 'txt' files, you must pass ".txt".
   */
   ...
   * one of the string specified in the constructor.
   */
  @Override
  public boolean accept(File dir, String filename) {
    for (int i = 0; i < endStrings.length; i++) {
      if (filename.endsWith(endStrings[i])) return true;
    }
    return false;
  }
}
