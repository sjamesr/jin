...
public class PlatformUtils {
  ...
  /**
   * Returns whether the version of Java we're running in is higher than or equal to the specified
   * version.
   */
  ...
  /**
   * Returns whether we're running under the old (just in case there's ever a new one) Microsoft VM.
   */
  public static boolean isOldMicrosoftVM() {
    String vendor = System.getProperty("java.vendor");

    return (vendor != null)
        && vendor.toLowerCase().startsWith("microsoft")
  ...
  public static boolean isSolaris() {
    ...
    return (os != null)
        && (os.toLowerCase().startsWith("solaris") || os.toLowerCase().startsWith("sunos"));
  }
  ...
  public static String getOSName() {
    if (isWindows() || isOldWindows()) return "windows";
    else if (isLinux()) return "linux";
    else if (isMacOSX()) return "macosx";
