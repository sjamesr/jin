...
public class StringParser {
  ...
  /**
   * Parses the given String as a Dimension object. The expected format is: "<width>;<height>" where
  ...
  /**
   * Parses the given string as a color. The expected format is a hexadecimal value in the range
   * [0..0xffffff] in RGB format.
   *
  ...
  public static String parseString(String s) {
    ...
    for (int i = 0; i < s.length(); i++) {
      char c = s.charAt(i);
      if (c == '\\') {
        if (buf == null) buf = new StringBuffer(s.substring(0, i)); // Initialize lazily.
        c = s.charAt(++i);
        switch (c) {
          case 'n':
            buf.append('\n');
