...
/**
 * Implements a raw connection to a server. The output from the server is written to the standard
 * output stream and input from the standard input stream is sent to the server.
 */
public class RawTelnet {
  ...
  private static class ProcessKillerThread extends Thread {
    ...
    /**
     * Waits for the target thread to die, prints a message and calls <code>System.exit(0)</code>
     */
    @Override
    public void run() {
      try {
