...
public abstract class Connection {
  ...
  /**
   * The username requested by the user. This may be different from the actual username assigned by
   * the server.
   */
  ...
  /**
   * The thread that connects to the server and then runs the read-loop, <code>null</code> until a
   * connection attempt is initiated.
   */
  ...
  /**
   * Creates a new <code>Connection</code> with the specified requested username and password. Both
   * the username and the password are kept merely as a convenience for subclasses - this class does
   * not use them in any manner other than returning them from {@link #getRequestedUsername()} and
   * {@link getPassword()}. Consequently they may both be <code>null</code>.
   */
  ...
  /**
   * Initiates a connection to the specified hostname on the specified port. The actual connecting
   * is done asynchronously.
   */
  public synchronized void initiateConnect(final String hostname, final int port) {
    readerThread =
        new Thread("ReaderThread") {
          @Override
          public void run() {
            try {
              if (connect(hostname, port)) readerLoop();
            } finally {
  ...
  /**
   * Creates the <code>InputStream</code> that will be passed to {@link #readMessage(InputStream)}.
   * This allows implementations to wrap the input stream in other input streams, which are useful
  ...
  /**
   * Reads and returns a single message from the server. Returns <code>null</code> if an EOF is read
   * from the server.
   */
  protected abstract Object readMessage(InputStream inputStream) throws IOException;

  /**
   * Invoked when an interruption occurs in the communication with the server. The specified
   * exception is the one that was thrown, which may be <code>null</code> if we were disconnected
   * cleanly from the server (end of stream was reached). Note that client code is also expected to
   * invoke this method if, for example, an <code>IOException</code> is thrown while trying to send
   * data to the server.
   */
  protected final synchronized void connectionInterrupted(final IOException exception) {
    ...
    try {
      socket
          .close(); // Closing the socket causes a SocketException to be thrown in the reader thread
      socket = null;
      username = null;
  ...
  /**
   * Sends the login sequence to the server via the specified output stream. This method must not
   * block.
   *
  ...
  /**
   * Closes this connection. If the connection is currently attempting to connect, the attempt is
   * aborted. If the connection is connected, disconnection is initiated. Otherwise, the call is
   * ignored. The actual disconnection is performed asynchronously.
   */
  public synchronized void close() {
    if (isConnected()) connectionInterrupted(null);
  ...
  /**
   * Client code (specifically {@link #handleMessage(Object)} invokes this method when it receives a
   * message from the server acknowledging login.
   *
  ...
  /**
   * Client code (specifically {@link #handleMessage(Object)} invokes this method when it receives a
   * message from the server indicating that it denied to log us in.
   *
  ...
  /**
   * Any invocation of client code (all the <code>handleXXX</code> methods) from the thread that
   * reads data from the server is done via this method. This allows clients to specify which thread
   * they want to handle connection events in, which is especially useful for Swing/AWT based
   * applications. The default implementation simply invokes the runnable, so the event handling is
   * done in the reader thread.
   */
  ...
  /**
   * Invoked when the attempt to connect to the server fails. The default implementation does
   * nothing.
   *
  ...
  /**
   * Invoked when a message arrives from the server. The specified message is an object returned by
   * the {@link #readMessage(InputStream)} method, so an implementation can cast it to the
   * appropriate type. It is up to this method to recognize when the server acknowledges or denies
   * login and invoke the {@link #loginSucceeded(String)} or {@link #loginFailed(String)} methods
   * accordingly.
   */
  protected abstract void handleMessage(Object message);

  /**
   * Invoked when the server acknowledges that we are logged in. The default implementation does
   * nothing.
   */
  ...
  /**
   * Invoked when we are disconnected from the server. The specified <code>IOException</code> is the
   * exception which caused us to believe we are disconnected. It is <code>null</code> in the case
   * of a clean disconnection (EOF read from the server). The default implementation does nothing.
   */
