...
public class AWTUtilities {
  ...
  /**
   * Packs and centers the given window relative to the given component. The specified component may
   * be <code>null</code>, in which case the window will be centered on the screen. The method also
   * makes sure that the target window is fully visible by calling <code>forceToScreen</code>.
   */
  ...
  /**
   * Reposition the specified window so that it necessarily fits on the screen, while trying to
   * minimize changes.
   */
  ...
  /**
   * Returns the parent Frame of the specified <code>Component</code> or <code>null</code> if none
   * exists.
   */
  ...
  /**
   * Returns a list of available font names. Under JDK1.1 it uses <code>Toolkit.getFontList()</code>
   * while under JDK1.2 (via reflection), <code>GraphicsEnvironment.getAvailableFontFamilyNames()
   * </code>
   */
    ...
    if (PlatformUtils.isJavaBetterThan("1.2")) {
      try {
        ...
        Method getLocalGraphicsEnvironmentMethod =
            geClass.getMethod("getLocalGraphicsEnvironment", new Class[0]);
        Object localGE = getLocalGraphicsEnvironmentMethod.invoke(null, new Object[0]);
        Method getAvailableFontFamilyNamesMethod =
            geClass.getMethod("getAvailableFontFamilyNames", new Class[0]);
        String[] fontNames =
            (String[]) getAvailableFontFamilyNamesMethod.invoke(localGE, new Object[0]);
        return fontNames;
      ...
      return null;
    } else return Toolkit.getDefaultToolkit().getFontList();
  }

  /**
   * Returns the state of the specified frame, as specified by <code>Frame.getExtendedState()</code>
   * if running under JDK 1.4 or later, otherwise returns 0. The call to <code>
   * Frame.getExtendedState()</code> is done via reflection to avoid runtime errors.
   */
  ...
  /**
   * Sets the state of the specified frame, as specified by <code>Frame.setExtendedState</code> if
   * running in JDK 1.4 or later, otherwise does nothing. The call to <code>Frame.setExtendedState()
   * </code> is done via reflection to avoid runtime errors.
   */
  public static void setExtendedFrameState(Frame frame, int state) {
    if (PlatformUtils.isJavaBetterThan("1.4")) {
      try {
        Class frameClass = Class.forName("java.awt.Frame");
        Method setExtendedStateMethod =
            frameClass.getMethod("setExtendedState", new Class[] {int.class});
  ...
  /**
   * Attempts to determine the usable screen bounds of the default screen device. If the require
   * java.awt API is not available under the JVM we're running in, this will simply return the
   * screen bounds obtained via <code>Toolkit.getScreenSize()</code>.
   */
  public static Rectangle getUsableScreenBounds() {
    if (PlatformUtils.isJavaBetterThan("1.4")) {
      try {
        ...
        Object defaultScreenDevice =
            getDefaultScreenDeviceMethod.invoke(graphicsEnvironment, emptyObjArr);
        Object defaultConfiguration =
            getDefaultConfigurationMethod.invoke(defaultScreenDevice, emptyObjArr);
        Rectangle bounds = (Rectangle) getBoundsMethod.invoke(defaultConfiguration, emptyObjArr);
        Insets insets =
            (Insets)
                getScreenInsetsMethod.invoke(
                    Toolkit.getDefaultToolkit(), new Object[] {defaultConfiguration});
   ...
   *
   * <p>This is a rather hacky method - it doesn't work well if there are both enabled and disabled
  ...
  /**
   * Returns whether <code>size1</code> fits inside <code>size2</code>, which it does iff both its
   * width and height are less than or equal to <code>size2</code>'s width and height, respectively.
   */
  ...
  /**
   * Adds the specified <code>Insets</code> to the specified <code>Dimension</code> and returns it.
   */
