...
public class TextUtilities {
  ...
  /**
   * Pads the beginning of the given String with the given character until it's <code>length</code>
   * characters long. If the given String's size is already <code>length</code> or larger, the given
  ...
  public static String padStart(String s, char c, int length) {
    ...
    for (int i = s.length(); i < length; i++) buf.insert(0, c);

    return buf.toString();
  }

  /**
   * Pads the end of the given String with the given character until it's <code>length</code>
   * characters long. If the given String's size is already <code>length</code> or larger, the given
   * string is returned as is.
   */
  public static String padEnd(String s, char c, int length) {
    ...
    StringBuffer buf = new StringBuffer(s);
    for (int i = s.length(); i < length; i++) buf.append(c);

    return buf.toString();
  }

  /**
   * Pads the given String on both sides equally (if possible) with the given character until it's
   * <code>length</code> characters long. If the given String's size is already <code>length</code>
   * or larger, the given string is returned as is.
   */
  public static String padSides(String s, char c, int length) {
    ...
    if (buf.length() < length) buf.insert(0, c);
  ...
  public static String trimRight(String s) {
    ...
    while (Character.isWhitespace(buf.charAt(length - 1))) buf.setLength(--length);
  ...
  /**
   * Returns an array of the tokens produced by the specified string with the specified delimiter
   * characters.
   */
  ...
  public static int[] parseIntList(String text, String delimiters) {
    ...
    for (int i = 0; i < tokens.length; i++) tokens[i] = Integer.parseInt(tokenizer.nextToken());

    return tokens;
  }

  /**
   * Translates the specified resource name into the context of the specified class. Basically, this
   * method returns the name of the resource you need to use when loading via a classloader, if via
   * the specified class you would load it simply with <code>Class.getResource(resourceName)</code>.
   */
  ...
  /**
   * Breaks the specified text into lines no longer than the specified length, without breaking
   * words. The breaking is done by inserting newlines at appropriate locations. Note that if there
   * are words longer than <code>maxLength</code>, they will not be broken.
   */
  ...
  /**
   * Returns the name of the system's default charset. This method is useful under 1.4, because
  ...
  public static String convert(String s, String fromEncoding, String toEncoding)
    ...
    if ((fromEncoding == null) || (toEncoding == null)) return s;
