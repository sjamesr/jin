...
/**
 * A class loader with a parent, to which loading is delegated if the class or resource can't be
 * found. This class isn't needed with JDK1.2 which already has this functionality. As an additional
 * feature, this class uses a Hashtable to look up already loaded classes, so any subclasses needn't
 * do this.
 */
public abstract class ChildClassLoader extends ClassLoader {
  ...
  /**
   * Loads and optionally resolves the class with the specified name. If this classloader can't find
   * the specified class, the parent is asked to load it.
   */
  @Override
  public final synchronized Class loadClass(String name, boolean resolve)
    ...
    if (c == null) {
      ...
      else c = parent.loadClass(name, resolve);
    }
  ...
  /**
   * Returns an <code>InputStream</code> for reading the resource with the specified name. If this
   * classloader can't find the resource, the parent is asked to find it.
   */
  @Override
  ...
  /**
   * Returns a <code>URL</code> pointing to the resource with the specified name. If this
   * classloader can't find the resource, the parent is asked to find it.
   */
  @Override
  ...
  /**
   * Loads the class data for the specified class name. The default implementation loads the class
   * data via <code>getResourceAsStreamImpl</code>
   */
  ...
  /**
   * Loads the class with the specified name and optionally resolves it. The default implementation
   * first tries to obtain the class via <code>findSystemClass</code> and if that fails, loads the
   * class via <code>loadClassData</code>.
   */
