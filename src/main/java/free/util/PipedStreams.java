...
/**
 * This class is responsible for creating and managing pairs of PipedStreams. A single instance of
 * this class consists of one PipedInputStream and one PipedOutputStream connected to each other.
 */
public class PipedStreams {
  ...
  /**
   * Creates new <code>PipedStreams</code> with the specified buffer size. Once the specified amount
   * of bytes have been written into the, <code>OutputStream</code> attempting to write more data
   * will block until enough data has been read to allow writing into the buffer again.
   */
  ...
  /**
   * Creates new <code>PipedStreams</code>. If <code>growBuf</code> is <code>true</code>, the
   * internal buffer will be grown indefinitely when more space is required for the written data.
   * This means that writing into the <code>OutputStream</code> will never block. Note that there is
   * currently no mechanism to cause the internal buffer to shrink.
   */
  ...
  /**
   * Creates new <code>PipedStreams</code> with the specified initial buffer size, potentially
   * allowing the buffer to grow indefinitely.
   */
  ...
  /**
   * Sets the SO_TIMEOUT for the PipedInputStream. A read operation on the PipedInputStream will
   * only block for the given amount of milliseconds, after that, an InterruptedIOException will be
   * thrown, but the streams will remain valid. A value of 0 implies this option is off (read() can
   * block indefinitely). NOTE: This method should not be called while a read() operation is in
   * progress (it will block until the read() is done, which may be a long time, or never).
   */
  ...
  /**
   * Returns the value of SO_TIMEOUT. A value of 0 implies this option is off (read() can block
   * indefinitely).
   */
  ...
  /**
   * Returns the amount of bytes available to be read immediately (without blocking) by the
   * PipedInputStream.
   */
  ...
  /**
   * Increases the size of the internal buffer by at least the specified amount of bytes. The caller
   * must take care of proper synchronization.
   */
  ...
  /**
   * Writes bytes according to the contract of OutputStream.write(byte [], int, int) with the only
   * difference that this might block if the buffer is full.
   */
  synchronized void write(byte[] arr, int offset, int length) throws IOException {
    synchronized (writeLock) {
      ...
      while (length > 0) {
        ...
        int part1Size =
            buf.length - writeIndex >= amountToWrite ? amountToWrite : buf.length - writeIndex;
  ...
  /**
   * Reads bytes from the buffer into the given array according to the contract of
