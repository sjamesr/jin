...
/**
 * A convenient storage place for Listeners. The listeners are stored in the same manner as in
 * javax.swing.event.EventListenerList. <STRONG>Note:</STRONG> This class is not thread safe.
 */
public class EventListenerList {
  ...
  /** Returns the amount of listeners of the specified type in this listener list. */
  public int getListenerCount(Class listenerType) {
    int count = 0;
    for (int i = 0; i < listenerList.length; i += 2) {
      if (listenerList[i] == listenerType) count++;
    }
    return count;
  }

  /**
   * Returns an array containing all the listeners in this listener list. The array contains
   * listeners and their types in the following format: the element at index 2*N is the type of the
   * listener at index 2*N+1 <B>WARNING!!!</B> Absolutely NO modification of the data contained in
   * this array should be made -- if any such manipulation is necessary, it should be done on a copy
  ...
  /**
   * Returns an array of listeners registered as the listeners of the given type. Note that the
   * returned array is an array of the actual given type so it can be safely casted to that type
   * once instead of casting each of the listener.
   */
  public EventListener[] getListeners(Class listenerType) {
    EventListener[] listeners =
        (EventListener[]) Array.newInstance(listenerType, getListenerCount(listenerType));
    int count = 0;
    for (int i = 0; i < listenerList.length; i += 2) {
      if (listenerType == listenerList[i]) listeners[count++] = (EventListener) listenerList[i + 1];
    }
    return listeners;
  }
}
