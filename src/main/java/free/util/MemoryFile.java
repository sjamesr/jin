...
public class MemoryFile {
  ...
  /**
   * Creates a new <code>MemoryFile</code> initialized with the data from the specified byte array.
   * The specified byte array is copied.
   */
  ...
  /**
   * Creates a new <code>MemoryFile</code> initialized with the data from the specified byte array
   * and the initial buffer size. Note that if the specified initial buffer size is smaller than the
   * length of the byte array, it will only be user when/if the contents of the file are cleared.
   */
  ...
  /**
   * Returns an <code>OutputStream</code> that will write into this <code>MemoryFile</code>. Only a
   * single open <code>OutputStream</code> is allowed per <code>MemoryFile</code>. Note that
  ...
  /**
   * Returns an <code>InputStream</code> that will read from this <code>MemoryFile</code>. Note that
   * the data read from the returned <code>InputStream</code> will be the data in the file when this
   * method is invoked. If more data is written into the file or the contents of the file are
   * cleared, the returned <code>InputStream</code> will not be affected.
   */
  ...
  public synchronized void clear() throws IOException {
    if (out != null) throw new IOException("MemoryFile open for writing");
    size = 0;
  }
  ...
  /**
   * Writes the specified amount of bytes from the specified array starting with the specified index
   * into the file.
   */
  private synchronized void write(byte[] arr, int offset, int length) {
    if (data.length - size < arr.length) growBuf(arr.length + size - data.length);
  ...
  private synchronized void closeOutputStream() throws IOException {
    if (out == null) throw new IOException("OutputStream already closed");
