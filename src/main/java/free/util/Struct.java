...
/**
 * This is a convenient superclass for classes whose job is to be containers of final (constant)
 * data. It's especially convenient for holding a large amount of fields, since it alleviates the
 * need for instance variables, instead holding the data in a Hashtable.
 */
public class Struct {
  ...
  /** Creates a new struct with the specified estimated amount of properties it will contain. */
  public Struct(int propertiesCount) {
    properties = new Hashtable(propertiesCount);
  }

  /**
   * Sets the value of the given property. A property may not have its value changed after it's been
   * set.
   *
   * <p><B>Known bug:</B> Since Hashtables don't allow <code>null</code> values, you may change the
   * value from <code>null</code> to any value.
   */
  protected final void setProperty(String propertyName, Object propertyValue) {
    Object oldValue;
    if (propertyValue != null) oldValue = properties.put(propertyName, propertyValue);
    else oldValue = properties.get(propertyName);
    if (oldValue != null) {
      properties.put(propertyName, oldValue);
      throw new IllegalArgumentException(
          "A property's value may not be reset - attempted to change the value of " + propertyName);
    }
  }
