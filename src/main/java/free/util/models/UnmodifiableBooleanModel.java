...
/**
 * An interface which presents a read-only API to a <code>BooleanModel</code>. This is useful if an
 * object wants to expose its state via a model, but forbid write access to it.
 */
public interface UnmodifiableBooleanModel {

  /** Returns the current state of the model. */
  boolean get();

  /** Returns whether the model's state is currently "on". */
  boolean isOn();

  /** Returns whether the model's state is currently "off". */
  boolean isOff();

  /** Adds a listener. */
  void addListener(BooleanListener l);
