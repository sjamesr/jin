...
/**
 * Lists and categorizes (some of) the encodings (charsets) available in the JVM.
 *
...
public class Encodings {
  ...
  static {
    try {
      Properties props =
          IOUtilities.loadProperties(Encodings.class.getResourceAsStream("charsets.properties"));
      for (Iterator categories = props.entrySet().iterator(); categories.hasNext(); ) {
        ...
        for (int i = 0; i < aliases.length; i++) {
          ...
          if (Charset.isSupported(alias)) encodings.add(Charset.forName(aliases[i]));
        }
  ...
  /**
   * Returns the list of encoding categories. The list is sorted by the category names in the
   * current application's locale.
   */
  ...
  /**
   * Returns a map from encoding category keys to sets of encodings (and their aliases) in that
   * category. For example, the <code>"western"</code> encoding category could be mapped to the set
  ...
  /**
   * Returns a map from encoding category keys to their names, in the current application's locale.
   */
