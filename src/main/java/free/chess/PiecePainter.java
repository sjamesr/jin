...
/** An interface for classes that paint the pieces (normally on a <code>JBoard</code>). */
public interface PiecePainter {

  /**
   * Paints the given piece at the given coordinates on the given <code>Graphics</code> object
   * scaled to the given size. The <code>component</code> argument may be null. The <code>shaded
   * </code> argument specifies whether the piece should be drawn in a special, shaded manner.
   */
  void paintPiece(Piece piece, Graphics g, Component component, Rectangle rect, boolean shaded);

  /**
   * Returns a new <code>PiecePainter</code> of the same class as this one. Implementations which
   * are immutable may return this instance itself instead of an actual copy.
   */
