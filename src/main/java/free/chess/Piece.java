...
public abstract class Piece {
  ...
  /**
   * An integer representing the piece. Negative values are black pieces and positive are white
   * pieces. The absolute value of this variable is the type of the piece. Thus, the value of this
   * integer is color*type;
   */
  ...
  protected Piece(int color, int type) {
    ...
    if (type == 0) throw new IllegalArgumentException("Piece type may not be 0");
  ...
  /**
   * Returns a short (notational) string representing this chess piece ("N" for a knight for
   * example). Black and white pieces should be represented by the same string. To differentiate
   * between black and white pieces, use the <code>toShortColorString</code> method.
   *
  ...
  /**
   * Returns the same as toShortString() only lowercase for black pieces and uppercase for white
   * ones.
   */
  public String toShortColorString() {
    if (isWhite()) return toShortString().toUpperCase();
    else return toShortString().toLowerCase();
  }

  /**
   * Returns a string representing the color of this piece, "White" for a white piece and "Black"
   * for a black piece. Returns "Colorless" for the empty piece.
   */
  ...
  /**
   * Returns true if the given Piece is the same as this one. This returns true if and only if the
   * type (class) of this piece is the same as the type of the given piece and their "piece" values
   * are the same. Subclasses implementing singleton pieces should override this and compare by
   * reference.
   */
  @Override
