...
public abstract class AbstractChessClock extends JComponent {
  ...
  /**
   * The time that was last set on this clock. Note that if the clock is running, the actual time is
   * calculated by subtracting from this value the amount of time that has passed since the clock
   * started running.
   */
  private int time;
  ...
  /**
   * The value of the system clock at the time the clock was set to run. -1 if the clock is not
   * running.
   */
  private long runStart = -1;

  /**
   * The current display mode of this clock. Possible values are {@link #HOUR_MINUTE_DISPLAY_MODE},
   * {@link #MINUTE_SECOND_DISPLAY_MODE) and {@link #SECOND_TENTHS_DISPLAY_MODE}.
   */
  ...
  /**
   * Creates a new <code>AbstractChessClock</code> with the given initial amount of time (in
   * milliseconds) on it.
   */
  ...
  /**
   * Sets the time thresholds under which the clock switches to display <code>minutes:seconds</code>
   * and <code>minutes:seconds.tenths</code>, when in <code>TIME_DEPENDENT_DISPLAY_MODE</code> mode.
   */
  ...
  /**
   * Sets the display mode of this clock. Possible values are {@link #HOUR_MINUTE_DISPLAY_MODE},
   * {@link #MINUTE_SECOND_DISPLAY_MODE) and {@link #SECOND_TENTHS_DISPLAY_MODE}. Note that
   * concrete implementations may ignore this setting.
   */
  ...
  protected int getActualDisplayMode() {
    ...
    if (displayMode != TIME_DEPENDENT_DISPLAY_MODE) return displayMode;

    int time = Math.abs(getTime());
    if (time < secondTenthsThreshold) return SECOND_TENTHS_DISPLAY_MODE;
    else if (time < minutesSecondsThreshold) return MINUTE_SECOND_DISPLAY_MODE;
    else return HOUR_MINUTE_DISPLAY_MODE;
  }

  /**
   * Sets the active flag of this clock. The clock should be set to be active when its owner is also
   * the owner of the current turn.
   */
