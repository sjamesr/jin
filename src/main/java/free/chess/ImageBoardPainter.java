...
/**
 * This class is provided for backwards compatibility with uses as a <code>ResourceBoardPainter
 * </code>. The actual drawing is now done by either {@link BoardImageBoardPainter} or {@link
...
public final class ImageBoardPainter implements ResourceBoardPainter {
  ...
  /** Since <code>ImageBoardPainter</code>s are immutable, simply returns <code>this</code>. */
  @Override
   ...
   *
   * <ul>
   *   <li><code>type</code>: The value is either "single" or "light-dark". This specifies whether
   ...
   * </ul>
   *
   * Based on the value of this property, the actual drawing is done by either a {@link
   * BoardImageBoardPainter} or a {@link SquareImagesBoardPainter}.
   */
  @Override
  public void load(URL url) throws IOException {
    ...
    if ("single".equals(type)) delegate = new BoardImageBoardPainter();
    else if ("light-dark".equals(type)) delegate = new SquareImagesBoardPainter();
    else throw new IOException("Unrecognized type value: " + type);
    delegate.load(url);
  }
