 ...
 * HREF="http://www.chessclub.com/help/Fischer-random">http://www
 * .chessclub.com/help/Fischer-random</A>
 */
public class FischerRandom extends ChesslikeGenericVariant {
   ...
   * according to the rules of the "fischer random" variant. Returns <code>false</code> otherwise.
   * The result for an illegal move is undefined, but it should throw no exceptions.
   */
  @Override
  public boolean isShortCastling(
    ...
    if (movingPiece == ChessPiece.WHITE_KING) {
      if (startingSquare.getRank() != 0) return false;
      else if ((takenPiece != null) && (takenPiece != ChessPiece.WHITE_ROOK)) return false;
      else if (!endingSquare.equals("g1")) return false;
      else if (startingSquare.equals("f1")) {
        if (takenPiece == ChessPiece.WHITE_ROOK) return true;
        else return false; // This *could* be a castling, but there is currently
      ...
      if (startingSquare.getRank() != 7) return false;
      else if ((takenPiece != null) && (takenPiece != ChessPiece.BLACK_ROOK)) return false;
      else if (!endingSquare.equals("g8")) return false;
      else if (startingSquare.equals("f8")) {
        if (takenPiece == ChessPiece.BLACK_ROOK) return true;
        else return false; // This *could* be a castling, but there is currently
        // no way to indicate whether it is one.
      }
  ...
  /**
   * Returns the square of the specified piece on the specified row, or null if the specified piece
   * is not on the specified row.
   */
  private static Square findPieceOnRow(Position pos, Piece piece, int rank) {
    for (int i = 0; i < 8; i++)
      if (piece.equals(pos.getPieceAt(i, rank))) return Square.getInstance(i, rank);

    return null;
  }

  /**
   * Initializes the given position to a random state subject to the constraints specified in the
   * rules.
   *
   * @throws IllegalArgumentException If the given Position's wild variant is not FischerRandom.
   */
  @Override
  ...
  /**
   * Creates a random initial position subject to the constraints specified in the rules of Fischer
   * Random. The position is encoded and returned in FEN format.
   */
  ...
  /**
   * Returns a random int, in the range [0..max). This method is used when creating the initial,
   * random position.
   */
  ...
  public void makeMove(Move move, Position pos, Position.Modifier modifier) {
    ...
    if (!(move instanceof ChessMove))
      throw new IllegalArgumentException(
          "The given move must be an instance of " + ChessMove.class.getName());
    ...
    } else super.makeMove(move, pos, modifier);
  }
}
