...
public class ShuffleBoth extends BothSidesCastlingVariant {
   ...
   * rules.
   *
   * @throws IllegalArgumentException If the given Position's wild variant is not ShuffleBoth.
   */
  @Override
  ...
  /**
   * Creates a random initial position subject to the constraints specified in the rules of Shuffle
   * Both. The position is encoded and returned in FEN format.
   */
  ...
  private static String createRandomPieceRow() {
    ...
    // The knights
    for (int i = 1; i < 7; i++) if (pieces.charAt(i) == '-') pieces.setCharAt(i, 'N');

    return pieces.toString();
  }

  /**
   * Randomly returns either <code>true</code> or false. This method is used when creating the
