...
/**
 * Implements a wild variant which differs from regular chess in that kings can be initially placed
 * on the d file in which case short castling refers to moving the king to the b file and the rook
 * to the c file. This class extends {@link ChesslikeGenericVariant} without defining the initial
...
public class BothSidesCastlingVariant extends ChesslikeGenericVariant {
  ...
  /**
   * Creates a new instance of <code>BothSideCastlingVariant</code> with the specified initial
   * position and name.
   */
  ...
  /**
   * Returns true if a move from the specified starting square to the specified ending square is a
   * short castling. This variant allows castling by kings from the d1 and d8 to the b1 and b8
   * squares (respectively) as well.
   */
  @Override
  public boolean isShortCastling(
      Position pos, Square startSquare, Square endSquare, ChessPiece promotionTarget) {
  ...
  /**
   * Returns <code>true</code> if a move from the specified starting square to the specified ending
   * square is a short castling. This variant allows castling by kings from the d1 and d8 to the f1
   * and f8 squares (respectively) as well.
   */
  @Override
  public boolean isLongCastling(
    ...
    if (movingPiece == ChessPiece.WHITE_KING) {
      if (startSquare.equals("e1")
          ...
          && (pos.getPieceAt("d1") == null)) return true;

      if (startSquare.equals("d1")
          && endSquare.equals("f1")
          && (pos.getPieceAt("h1") == ChessPiece.WHITE_ROOK)
          && (pos.getPieceAt("e1") == null)
          && (pos.getPieceAt("g1") == null)) return true;
      ...
      if (startSquare.equals("e8")
          ...
          && (pos.getPieceAt("d8") == null)) return true;
  ...
  public Move createShortCastling(Position pos) {
    ...
    if (currentPlayer.isWhite()) {
      ...
      if (pos.getPieceAt(Square.parseSquare("e8")) == ChessPiece.BLACK_KING)
        ...
        return new ChessMove(
            Square.parseSquare("d8"),
            Square.parseSquare("b8"),
            Player.BLACK_PLAYER,
            false,
            true,
            false,
