 ...
 * href="http://www.chessclub.com/help/Shatranj">http://www.chessclub.com/help/Shatranj</A> for a
 * description of Shatranj.
 *
 * @author Ryan Propper
...
public class Shatranj extends NoCastlingVariant {
  ...
  /**
   * If the move created by the given starting square and ending square in the given position is a
   * promotion, returns an array containing a fers (queen) of the color of the promoted pawn.
  ...
  /**
   * Since en passant is not possible in Shatranj, this method always returns <code>false</code>.
   */
  @Override
  ...
  /**
   * Fers (queen) move directions. The fers can only move one square diagonally in any direction.
   */
  ...
  /**
   * Elephant (bishop) move directions. The elephant moves two squares at a time diagonally, and can
   * jump over a piece on the intermediate square.
   */
  ...
  /**
   * Returns target squares for a pawn. In shatranj there is no double pawn push and no en-passant.
   */
  private Collection getShatranjPawnTargetSquares(Position pos, Square square) {
    ...
    for (Iterator i = chessPawnTargets.iterator(); i.hasNext(); ) {
      ...
      if (chess.isEnPassant(pos, square, targetSquare, null)) continue;

      // No double pawn push in Shatranj
      if (chess.getDoublePawnPushFile(pos, square, targetSquare) != -1) continue;
