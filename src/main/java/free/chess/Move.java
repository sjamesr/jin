...
public abstract class Move {
  ...
  /**
   * Creates a new Move from the given starting Square to the given ending Square, with the given
   * moving Player.
   *
   * <p>Both the starting and ending squares may be null, as they sometimes are, for example in
   * Kriegspiel.
   *
   * <p>The given stringRepresentation will be the string returned from the toString() method. It
   * may be <code>null</code>, in which case a string constructed from the move data will be
   * returned by toString().
   */
  public Move(
      Square startingSquare, Square endingSquare, Player player, String stringRepresentation) {

    if (player == null) throw new IllegalArgumentException("The moving player may not be null");
  ...
  /** Returns the initial square of the moving piece or <code>null</code> if unknown. */
  public Square getStartingSquare() {
    return startingSquare;
  }

  /** Returns the square to which the moving piece moved or <code>null</code> if unknown */
  ...
  /**
   * Breaks the given move string, represented in Smith Warren format into starting square, ending
   * square and promotion target, and using WildVariant.parsePiece(String) and
   * WildVariant.createMove(Position, Square, Square, Piece) creates a Move object.
   */
  ...
  public String toString() {
    if (getStringRepresentation() == null) return getMoveString();
    else return getStringRepresentation();
  }
}
