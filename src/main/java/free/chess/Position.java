 ...
 * of pieces, there being two players and other things common to most chess variants.
...
public final class Position {
  ...
  /**
   * Puts the given piece at the given square, replacing the piece that was there before.
   *
  ...
  /**
   * Sets this Position to represent the position represented by the given string. The string should
   * represent a position by specifying 64 characters indicating what occupies (-PNBRQKpnbrqk) each
   * square, in lexigraphic order (a8, b8, ..., h1). The player to move will be the player with the
   * white pieces.
   *
  ...
  /**
   * Returns a string representing the position by 64 characters indicating what occupies (obtained
   * by calling toShortColorString() on each piece, '-' for no piece) each square, in lexigraphic
   * order (a8, b8, ..., h1).
   */
  public String getLexigraphic() {
    ...
    for (int rank = 7; rank >= 0; rank--)
      for (int file = 0; file < 8; file++) {
        ...
        if (piece == null) buf.append("-");
        else buf.append(piece.toShortColorString());
      }
  ...
  /**
   * Sets this Position to represent the position described by the given string in FEN format. The
   * FEN format is described at <A
   * HREF="http://www.very-best.de/pgn-spec.htm#16.1">http://www.very-best.de/pgn-spec.htm#16.1</A>.
   * The characters describing pieces aren't limited to the chess set (like in FEN), but are
   * determined by the WildVariant of this position.
   *
   * @throws PositionFormatException if the given string is not in the expected format.
   */
  public void setFEN(String fen) throws PositionFormatException {
    ...
    if (ranks.countTokens() != 8) throw new PositionFormatException("Wrong amount of ranks");
    ...
    for (int rank = 7; rank >= 0; rank--) {
      ...
      if (file != 8) throw new PositionFormatException("Rank " + rank + " is a few files short");
    }

    String colorToMove = fenTokenizer.nextToken();
    if (colorToMove.length() != 1)
      throw new PositionFormatException(
          "Wrong amount of characters in active color indicator: " + colorToMove);
    if (colorToMove.equals("w")) setCurrentPlayerImpl(Player.WHITE_PLAYER);
    else if (colorToMove.equals("b")) setCurrentPlayerImpl(Player.BLACK_PLAYER);
    else throw new PositionFormatException("Wrong active color indicator: " + colorToMove);

    this.positionFEN = fen;
  }

  /**
   * Returns the FEN representation of this Position. May return <code>null</code> if the current
   * position wasn't set via the setFEN method. Note that as soon as the position is changed after
   * setFEN was called, this method will return <code>null</code>.
   */
  ...
  /**
   * Clears this position of any pieces. The current player is set to the player with the White
   * pieces.
   */
  public void clear() {
    for (int file = 0; file < 8; file++)
      for (int rank = 0; rank < 8; rank++) setPieceAtImpl(null, Square.getInstance(file, rank));
    setCurrentPlayerImpl(Player.WHITE_PLAYER);
  ...
  /**
   * Makes the given Move on this position. This method first fires a MoveEvent and then a
   * ChangeEvent.
   *
  ...
  /**
   * Makes this position a copy of the given position by setting it to the same state. The
   * WildVariants of the Positions must match.
   *
  ...
  /**
   * Puts the given piece at the given square, replacing the piece that was there before. The
   * difference between this and the setPieceAt(Piece,Square) method is that this method does not
   * fire a ChangeEvent. It's useful when you want to do a series of changes and only then fire a
   * ChangeEvent.
   *
   * @param piece The piece to put.
   * @param square The square where to put the piece.
  ...
  /**
   * Sets the current player in this position to be the given player. The difference between this
   * and the setCurrentPlayer(Player) method is that this method does not fire a ChangeEvent. It's
   * useful when you want to do a series of changes and only then fire a ChangeEvent.
   *
   * @param player The player whose turn it is next.
   */
  ...
  /**
   * Adds a ChangeListener. The change listeners are run each time the Position changes.
   *
  ...
  /**
   * Adds a MoveListener. The change listeners are run each time a move is made in this position.
   *
   * @param l the ChangeListener to add
   * @see #removeChangeListener
   */
  ...
  /**
   * Returns the squares to which the piece at the specified square may move. The set is not
   * guaranteed to contain only legal moves, but it is guaranteed that every legal move by the piece
   * has one of the squares as the target square.
   *
  ...
  public boolean equals(Position pos) {
    ...
    if (!currentPlayer.equals(pos.currentPlayer)) return false;

    for (int file = 0; file < pieces.length; file++)
      for (int rank = 0; rank < pieces[file].length; rank++)
        if (!Utilities.areEqual(pieces[file][rank], pos.pieces[file][rank])) return false;

    return true;
  }

  /**
   * Returns true iff the specified object is a <code>Position</code> and represents the same
  ...
  /**
   * Instances of this class is allowed to make modifications to a Position without triggering the
   * Position instance to fire any events. It should only be used by WildVariant implementations for
   * making multi-step atomic changes which don't trigger firing many events to a Position and then
   * firing an event indicating the change is done.
   */
  public static final class Modifier {

    /**
     * The Position we're modifying. Don't eliminate this by making the Modifier class nonstatic
     * because it causes Jikes to create invalid bytecode. See
     * http://www-124.ibm.com/pipermail/jikes-dev/2000-November/001585.html
     */
