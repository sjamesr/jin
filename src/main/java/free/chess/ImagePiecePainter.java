...
public final class ImagePiecePainter implements ResourcePiecePainter {
  ...
  /**
   * True if piece images are to be loaded asynchronously, and in the meanwhile, the delegate should
   * be used.
   */
  ...
  public ImagePiecePainter(Map pieceImages) {
    ...
    while (sizes.hasNext()) {
      ...
      if (size <= 0) throw new IllegalArgumentException("Image sizes must be positive");

      if (size > maxSize) maxSize = size;
    }

    if (maxSize == 0) throw new IllegalArgumentException("No sizes in the hashtable");
  ...
  /**
   * Sets whether piece images are to be loaded asynchronously and a simpler piece painter delegate
   * used while they load. Asynchronous loading is off by default.
   */
  ...
  public void load(URL baseUrl) throws IOException {
    ...
    if (def == null) throw new IOException("Unable to load " + defURL);
  ...
  protected int bestFitImageSize(int squareSize) {
    ...
    if (squareSize >= pieceImages.length) return pieceImages.length - 1;

    if (pieceImages[squareSize] != null) return squareSize;

    for (int i = squareSize; i > 0; i--) if (pieceImages[i] != null) return i;

    for (int i = squareSize + 1; i < pieceImages.length; i++) if (pieceImages[i] != null) return i;
  ...
  protected synchronized Map loadPieces(int squareSize, boolean shaded, Component target) {
    ...
    return images;
      ...
      ImageDataReceiver receiver =
          (ImageDataReceiver) imageDataReceivers.get(new Integer(imageSize));
      ...
      receiver =
          new ImageDataReceiver(
              asyncImageLoad ? target : null, imageSize, normalImages, shadedImages);
  ...
  /**
   * Paints the image of the given piece at the given coordinates on the given Graphics object
  ...
  /**
   * The receiver of loaded image data. Responsible for creating and correctly mapping the piece
   * images.
   */
  private class ImageDataReceiver implements IOUtilities.DataReceiver {
    ...
    /**
     * Creates a new <code>ImageDataReceiver</code> with the specified maps to fill and component to
     * repaint when loading is done.
     */
    public ImageDataReceiver(
        Component componentToRepaint, int imageSize, Map normalImages, Map shadedImages) {
