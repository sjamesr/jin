...
public class JBoard extends JComponent {
  ...
  /**
   * The constant for unified move input style. This style allows both drag'n'drop and click'n'click
   * move gestures, similarly to how menus work on many graphical systems.
   */
  ...
  /**
   * The constant for move input mode which only lets the user move the pieces of the player to
   * move.
   */
  ...
  /**
   * The constant for move highlighting done by highlighting the source and target squares of the
   * move.
   */
  public static final int BOTH_SQUARES_MOVE_HIGHLIGHTING = 2;

  /**
   * The constant for move highlighting done by drawing an arrow from the source square to the
   * target square.
   */
  ...
  /**
   * The constant for displaying row and column coordinates on the outer rim of the board, xboard
   * style.
   */
  ...
  /**
   * A copy of the current position on the board. We keep this so that when the real position
   * changes, we know which squares need repainting.
   */
  ...
  /**
   * A flag we set when the position was modified because of a move (not just a general change) so
   * we know to ignore the next change event.
   */
  ...
  /**
   * Updates the board from the specified current position to the specified target position, causing
   * the required areas to be repainted.
   */
  private void updateBoard(Position startPosition, Position endPosition) {
    ...
    if (checkMovingPieceSquare) repaint(tmpRect = getMoveAreaRect(tmpRect));
    ...
    for (int file = 0; file < 8; file++) {
      for (int rank = 0; rank < 8; rank++) {
        ...
        if (checkMovingPieceSquare
            && (file == movedPieceSquare.getFile())
            && (rank == movedPieceSquare.getRank())) {
  ...
  private final MoveListener positionMoveListener =
      new MoveListener() {
        ...
        public void moveMade(MoveEvent evt) {
          if (isMakingUserMove) return;

          if (!(evt.getMove().getClass().equals(ChessMove.class))) // We can't slide for such moves
          return;

          if (getSlideDuration() < 0) return;

          // Finish up the previous animation, if any
          if (slideStartSquare != null) updateBoard(positionCopy, slideTargetPosition);

          ChessMove move = (ChessMove) evt.getMove();

          positionChangedByMove = true;

          if (slideTargetPosition == null) slideTargetPosition = new Position(position);
          else slideTargetPosition.copyFrom(position);
  ...
  /**
   * The color used for coordinate display. <code>null</code> means the default label color is used.
   */
  ...
  /**
   * The location of the cursor when a move is being made; <code>null</code> when not making a move.
   */
  private Point movedPieceLoc = null;

  /**
   * The current target square during a move, <code>null</code> if none. Note that this may be
   * <code>null</code> even during a move (for example, if the snap-to-legal-square mechanism can't
  ...
  /**
   * How far along are we in sliding. The time passed from <code>slideStartTime</code> to the time
   * repaint() was called with the purpose of painting the next slide frame.
   */
  ...
  /**
   * A boolean telling us whether we're currently showing the promotion target selection dialog.
   * This is needed to workaround the bug which keeps sending events after show() has been called on
   * a modal dialog but before it's actually displayed.
   */
  ...
  public JBoard(Position position) {
    this(
        position,
        position.getVariant().createDefaultBoardPainter(),
  ...
  /**
   * Adds the given PaintHook to the list of PaintHooks which are called during the painting of this
   * JBoard.
   */
  public void addPaintHook(PaintHook hook) {
    if (paintHooks == null) paintHooks = new Vector(2);

    paintHooks.addElement(hook);
  }

  /**
   * Removes the given PaintHook from the list of PaintHooks which are called during the painting of
   * this JBoard.
   */
  ...
  public void setPosition(Position newPosition) {
    ...
    if (positionCopy == null) positionCopy = new Position(position);
    else positionCopy.copyFrom(position);
  ...
  public void setMoveHighlightingStyle(int newStyle) {
    ...
    if (highlightedMove != null) repaint();
  ...
  /**
   * Calculates the area that needs to be repainted for the current highlighting and repaints it.
   */
  private void repaintHighlighting() {
    int moveHighlightingStyle = getMoveHighlightingStyle();
    if ((moveHighlightingStyle == NO_MOVE_HIGHLIGHTING) || (highlightedMove == null)) return;
    ...
    if (moveHighlightingStyle == TARGET_SQUARE_MOVE_HIGHLIGHTING) repaint(squareToRect(to, null));
  ...
  public void setShowShadowPieceInTargetSquare(boolean newValue) {
    ...
    if (isMovingPiece()) repaint(squareToRect(targetSquare, null));
  ...
  public void setHighlightLegalTargetSquares(boolean newValue) {
    ...
    if (isMovingPiece()) {
      if (newValue) legalTargetSquares = position.getTargetSquares(movedPieceSquare);
      else legalTargetSquares = null;
  ...
  public void setLegalTargetSquaresHighlightColor(Color color) {
    ...
    if (legalTargetSquares != null) repaintLegalTargetSquares(null);
  ...
  /**
   * Sets the coordinate display style. Possible values are {@link #NO_COORDS}, {@link #RIM_COORDS},
   * {@link #OUTSIDE_COORDS} and {@link #EVERY_SQUARE_COORDS}.
   */
  ...
  /**
   * Sets the board's flipped state. When the board is flipped, it displays the black side at the
   * bottom.
   */
  ...
  /**
   * Sets whether the user will be prompted which piece to promote to when a promotion occurs or
   * will the default promotion piece be used.
   */
  ...
  /**
   * Returns true if on a promotion move, the user will be prompted which piece he wants to promote
   * to, returns false if the default promotion piece will be used without prompting the user.
   */
  ...
  /**
   * Sets the BoardPainter for this JBoard. Passing <code>null</code> is equivalent to setting the
   * board painter to the default one specified by the current position's variant.
   */
  public void setBoardPainter(BoardPainter boardPainter) {
    if (boardPainter == null) boardPainter = position.getVariant().createDefaultBoardPainter();
  ...
  /**
   * Sets the PiecePainter for this JBoard. Passing <code>null</code> is equivalent to setting the
   * piece painter to the default one specified by the current position's variant.
   */
  public void setPiecePainter(PiecePainter piecePainter) {
    if (piecePainter == null) piecePainter = position.getVariant().createDefaultPiecePainter();
  ...
  /**
   * Sets the color used for move highlighting to the specified color. Passing <code>null</code> is
   * equivalent to setting the color to the default one. Note: this refers to highlighting the move
   * specified by setHighlightedMove, not the move being made by the user.
   */
  public void setMoveHighlightingColor(Color moveHighlightingColor) {
    if (moveHighlightingColor == null) moveHighlightingColor = DEFAULT_MOVE_HIGHLIGHT_COLOR;
  ...
  /**
   * Sets the color used for coordinate display. Passing <code>null</code> is equivalent to setting
   * the color to the default one.
   */
  public void setCoordsDisplayColor(Color coordsDisplayColor) {
    if (coordsDisplayColor == null) coordsDisplayColor = DEFAULT_COORDS_DISPLAY_COLOR;
  ...
  /**
   * Returns the duration of piece slide animation. A negative value indicates that no sliding
   * occurs.
   */
  ...
  /**
   * Sets the duration of piece slide animation. A negative value indicates that no sliding should
   * occur.
   */
  ...
  public void clearShaded() {
    ...
    for (int file = 0; file < 8; file++)
      for (int rank = 0; rank < 8; rank++) {
        ...
        if (oldState) repaint(squareToRect(file, rank, helpRect));
      }
  }
  ...
  public void paintComponent(Graphics graphics) {
    ...
    for (int file = 0; file < 8; file++) {
      for (int rank = 0; rank < 8; rank++) {
        ...
        if (isPieceFollowsCursor && curSquare.equals(movedPieceSquare)) continue;

        if (isShowShadowPieceInTargetSquare && curSquare.equals(targetSquare)) continue;

        if (curSquare.equals(slideStartSquare)) continue;
    ...
    if (isHighlightLegalTargetSquares && (legalTargetSquares != null)) {
      ...
      for (Iterator i = legalTargetSquares.iterator(); i.hasNext(); ) {
        ...
        if (rect.intersects(clipRect)) g.fill(rect);
      }
    }
    ...
    if ((moveHighlightingStyle != NO_MOVE_HIGHLIGHTING) && (highlightedMove != null)) {
      ...
      if ((from != null) && (to != null)) {
        if (moveHighlightingStyle == BOTH_SQUARES_MOVE_HIGHLIGHTING) {
          drawSquare(
              g, from, highlightSize - Math.max(1, highlightSize / 3), getMoveHighlightingColor());
  ...
  /**
   * Returns the rectangle in which the sliding piece should be drawn at the specified progress in
   * the sliding (a number between 0 and 1).
   */
  ...
  /**
   * The font we use for drawing coordinates. The size might be different in the actual drawing
   * though.
   */
  ...
  private void drawCoords(Graphics g) {
    ...
    switch (getCoordsDisplayStyle()) {
      ...
      case OUTSIDE_COORDS:
        ...
        break;
      case EVERY_SQUARE_COORDS:
        drawEverySquareCoords(g);
        break;
      default:
        throw new IllegalStateException(
            "Unknown coordinates display style value: " + getCoordsDisplayStyle());
    }
  }
  ...
  private void drawRimCoords(Graphics g) {
    ...
    if (clipRect.intersects(
        new Rectangle(boardRect.x, boardRect.y, squareWidth / 2, boardRect.height))) {
      char row = isFlipped() ? '1' : '8';
      for (int i = 0; i < 8; i++) {
        g.drawString(
            String.valueOf(row), boardRect.x + 3, boardRect.y + i * squareHeight + fontHeight);
        row += dir;
      }
    }
    ...
    if (clipRect.intersects(
      ...
      for (int i = 0; i < 8; i++) {
        g.drawString(
            String.valueOf(col),
            boardRect.x + (i + 1) * squareWidth - fontWidth - 3,
            boardRect.y + boardRect.height - 3);
        col -= dir;
      }
    }
  }
  ...
  private void drawOutsideCoords(Graphics g) {
    ...
    // Row coordinates
    if (clipRect.intersects(
        new Rectangle(insets.left, insets.top, boardRect.x - insets.left, boardRect.height))) {
      char row = isFlipped() ? '1' : '8';
      for (int i = 0; i < 8; i++) {
        g.drawString(
            String.valueOf(row),
    ...
    if (clipRect.intersects(
        new Rectangle(
            boardRect.x, boardRect.y + boardRect.height, boardRect.width, bottomBorderHeight))) {
      char col = isFlipped() ? 'h' : 'a';
      for (int i = 0; i < 8; i++) {
        g.drawString(
            String.valueOf(col),
  ...
  protected void drawSquare(Graphics graphics, Square circleSquare, int size, Color color) {
    ...
    if (PlatformUtils.isMacOSX()) path.transform(AffineTransform.getTranslateInstance(-0.5, -0.5));
  ...
  /**
   * Returns the rectangle actually occupied by the board, without the borders or anything else that
   * surrounds the board.
   */
  ...
  private Rectangle getMovedPieceGraphicRect(Rectangle rect) {
    ...
    if (movedPieceLoc == null) throw new IllegalStateException("No piece is being moved");
  ...
  private Square calcTargetSquare(Point cursorLocation) {
    ...
    } else return locationToSquare(cursorLocation);
  }

  /**
   * Returns a rectangle (in pixels) which completely covers the area that needs to be redrawn when
   * a piece is being moved. Throws an <code>IllegalStateException</code> if a piece is not
  ...
  private Rectangle getMoveAreaRect(Rectangle rect) {
    ...
    if (isPieceFollowsCursor()) {
      if (isHighlightMadeMoveSquares() || isShowShadowPieceInTargetSquare()) {
        ...
        if (targetRect == null) rect.setBounds(pieceGraphic);
        else rect.setBounds(pieceGraphic.union(targetRect));

        return rect;
      } else return getMovedPieceGraphicRect(rect);
    } else {
      if (isHighlightMadeMoveSquares() || isShowShadowPieceInTargetSquare())
        return squareToRect(targetSquare, rect);
      else {
        if (rect == null) return new Rectangle(0, 0, 0, 0);
  ...
  /**
   * Returns the square corresponding to the given coordinate (in pixels). Returns null if the given
   * location is not on the visible board.
   */
  public Square locationToSquare(int x, int y) {
    ...
    if ((file > 7) || (rank < 0)) return null;

    if (isFlipped()) return Square.getInstance(7 - file, 7 - rank);
  ...
  /**
   * Returns whether we are currently performing the specified gesture or are simply in the
   * specified move input style. This is just a helper method.
   */
  ...
  /**
   * If a piece is currently being moved/dragged, the moving/dragging is canceled and the
   * moved/dragged piece is returned to its original location. If no piece is currently being
   * moved/dragged, an <code>IllegalStateException</code> is thrown.
   */
  public void cancelMovingPiece() {
    if (!isMovingPiece()) throw new IllegalStateException();

    Rectangle rect = new Rectangle();
    repaint(getMoveAreaRect(rect));
    repaint(squareToRect(movedPieceSquare, rect));
    if (targetSquare != null) repaint(squareToRect(targetSquare, rect));
    if (legalTargetSquares != null) repaintLegalTargetSquares(rect);
  ...
  protected void processMouseEvent(MouseEvent evt) {
    ...
    if (square == null) {
      if ((evtID == MouseEvent.MOUSE_RELEASED) && isMovingPiece()) cancelMovingPiece();
      return;
    }

    if (isLeftMouseButton
        && ((evtID == MouseEvent.MOUSE_PRESSED)
      ...
      if (movedPieceSquare == null) {
        ...
        if (evtID == MouseEvent.MOUSE_RELEASED) return;
        ...
        repaint(getMoveAreaRect(helpRect));
        if (isHighlightLegalTargetSquares()) repaintLegalTargetSquares(helpRect);
        ...
        if (targetSquare == null) { // Moved to a non-target square
          ...
          Piece[] promotionTargets =
              variant.getPromotionTargets(position, movedPieceSquare, targetSquare);
          Move madeMove;
          if (promotionTargets != null) {
            ...
            if (isManualPromote()) {
              ...
              promotionTarget =
                  PieceChooser.showPieceChooser(
                      this, x, y, promotionTargets, getPiecePainter(), promotionTargets[0]);
              isShowingModalDialog = false;
            } else promotionTarget = promotionTargets[0];

            madeMove =
                variant.createMove(position, movedPieceSquare, targetSquare, promotionTarget, null);
          } else
        ...
        if (targetSquare != null) repaint(squareToRect(targetSquare, helpRect));
  ...
  protected void processMouseMotionEvent(MouseEvent evt) {
    ...
    if ((evtID == MouseEvent.MOUSE_DRAGGED) && !SwingUtilities.isLeftMouseButton(evt)) return;

    if (movedPieceSquare == null) return;
