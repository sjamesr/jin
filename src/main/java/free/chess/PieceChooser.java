...
public class PieceChooser extends JComponent implements ActionListener {
  ...
  /**
   * Displays a modal dialog over the given Component at the specified coordinates, which lets the
   * user choose from the given set of Pieces which are painted using the given PiecePainter.
   * Returns the piece chosen by the user.
   */
  public static Piece showPieceChooser(
    ...
    final JDialog dialog =
        new JDialog(JOptionPane.getFrameForComponent(parentComponent), "Choose a piece", true);
    ...
    int locY =
        parentLoc.y
            + y
            - defaultButton.getY()
            - defaultButton.getHeight() / 2
            - 20; // 20 is what we think is a typical size for the window title.
  ...
  /**
   * Adds an ActionListener to this PieceChooser. An ActionEvent is fired when a piece is chosen.
   */
  ...
  protected void fireActionPerformed(ActionEvent evt) {
    ...
    ActionEvent e =
        new ActionEvent(
            this, ActionEvent.ACTION_PERFORMED, evt.getActionCommand(), evt.getModifiers());
