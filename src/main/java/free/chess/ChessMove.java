...
/**
 * An extension of the Move class for variants which have rules similar to regular chess. Mainly, it
 * just defines the existance of concepts such as en-passant, short and long castling, captures and
 * promotion. It provides two constructors, one which lets the caller to specify all the properties
 * and another one which determines these properties by using the stardard chess rules.
 */
public class ChessMove extends Move {
  ...
  /**
   * Creates a new ChessMove with the given properties. If the move is not a promotion, the
   * promotion target should be <code>null</code>. The <code>moveSAN</code> (the move in SAN format)
   * argument may be <code>null</code>. <code>doublePawnPushFile</code> should be -1 if the move
  ...
  public ChessMove(
    ...
    if (endingSquare == null) throw new IllegalArgumentException("Ending square may not be null");
  ...
  /**
   * Creates a new ChessMove from the given properties. The missing information (en-passant,
   * castling and other information provided by the more complete constructor) is determined
   * according to the rules of standard chess. The <code>moveSAN</code> (the move in SAN format)
  ...
  public ChessMove(
    ...
    this.isShortCastling =
        chess.isShortCastling(pos, startingSquare, endingSquare, promotionTarget);
    this.isLongCastling = chess.isLongCastling(pos, startingSquare, endingSquare, promotionTarget);
    this.capturedPiece =
        chess.getCapturedPiece(pos, startingSquare, endingSquare, promotionTarget, isEnPassant);
    this.doublePawnPushFile = chess.getDoublePawnPushFile(pos, startingSquare, endingSquare);
  }

  /**
   * Returns a string representing this move in the notation invented by Warren Smith. Refer to <A
   * HREF
   * ="http://www.chessclub.com/chessviewer/smith.html">http://www.chessclub.com/chessviewer/smith
   * .html</A> for a description of the notation.
   *
  ...
  public String getWarrenSmithString() {
    ...
    if (isEnPassant()) buf.append('E');
    else if (isShortCastling()) buf.append('c');
    else if (isLongCastling()) buf.append('C');
    else if (isCapture()) {
      ChessPiece capturedPiece = getCapturedPiece();
      if (capturedPiece.isPawn()) buf.append('p');
      else if (capturedPiece.isKnight()) buf.append('n');
      else if (capturedPiece.isBishop()) buf.append('b');
      else if (capturedPiece.isRook()) buf.append('r');
      else if (capturedPiece.isQueen()) buf.append('q');
      else if (capturedPiece.isKing()) buf.append('k');
    }

    if (isPromotion()) {
      ChessPiece promotionTarget = getPromotionTarget();
      if (promotionTarget.isKnight()) buf.append('N');
      else if (promotionTarget.isBishop()) buf.append('B');
      else if (promotionTarget.isRook()) buf.append('R');
      else if (promotionTarget.isQueen()) buf.append('Q');
    }
  ...
  /**
   * Returns the SAN representation of the move, or <code>null</code> if none was specified in the
   * constructor.
   */
  ...
  /**
   * Returns the piece to which the moving pawn was promoted, or null if this move is not a
   * promotion.
   */
  ...
  /**
   * Returns the double pawn push file of this move, or -1 if this move isn't a double pawn push.
   */
  ...
  public String getMoveString() {
    ...
    else if (isLongCastling()) return "O-O-O";
    else {
      String moveString = getStartingSquare().toString() + getEndingSquare().toString();
      if (isPromotion()) return moveString + "=" + getPromotionTarget().toShortString();
      else return moveString;
    }
  }
}
